...
final class ForgeOriginalVoxyNodeStore {
    ...
    static final int MAX_GEOMETRY_ID = (1 << 24) - 3;
    ...
    ForgeOriginalVoxyNodeStore(int maxNodeCount) {
    ...
    void copyNode(int fromId, int toId) {
        if (!this.allocationSet.isSet(fromId) || !this.allocationSet.isSet(toId)) {
            throw new IllegalArgumentException();
        }
        int from = id2idx(fromId);
        int to = id2idx(toId);
        this.localNodeData[to] = this.localNodeData[from];
        this.localNodeData[to + 1] = this.localNodeData[from + 1];
        this.localNodeData[to + 2] = this.localNodeData[from + 2];
        this.localNodeData[to + 3] = this.localNodeData[from + 3];
    }

    void setNodePosition(int node, long position) {
        this.localNodeData[id2idx(node)] = position;
    }

    long nodePosition(int nodeId) {
    ...
    void setNodeGeometry(int node, int geometryId) {
    ...
    void setChildPtr(int nodeId, int ptr) {
    ...
    void setNodeRequest(int node, int requestId) {
    ...
    void setNodeType(int nodeId, int type) {
        type >>>= 30;
        int idx = id2idx(nodeId) + 1;
        long data = this.localNodeData[idx];
        data &= ~(3L << 61);
    ...
    byte getNodeChildExistence(int nodeId) {
        long data = this.localNodeData[id2idx(nodeId) + 1];
        return (byte) ((data >> 48) & 0xFF);
    }

    void setNodeChildExistence(int nodeId, byte existence) {
        int idx = id2idx(nodeId) + 1;
        long data = this.localNodeData[idx];
        data &= ~(0xFFL << 48);
    ...
    int getChildPtrCount(int nodeId) {
        long data = this.localNodeData[id2idx(nodeId) + 1];
        return ((int) ((data >> 56) & 0x7)) + 1;
    }

    void setChildPtrCount(int nodeId, int count) {
        if (count <= 0 || count > 8) {
    ...
    void writeNode(long ptr, int nodeId) {
        if (!this.nodeExists(nodeId)) {
            MemoryUtil.memPutLong(ptr, -1L);
            MemoryUtil.memPutLong(ptr + Long.BYTES, -1L);
            return;
        }
        ...
        flags |= (short) (this.getAllChildrenAreLeaf(nodeId) ? 1 << 5 : 0);
