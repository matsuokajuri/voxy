...
final class ForgeOriginalVoxyMipGen {
    ...
    static MemoryBuffer putTexturesBuffer(boolean darkened, ForgeOriginalVoxyColourDepthTextureData[] textures) {
        MemoryBuffer buffer = new MemoryBuffer(ORIGINAL_MODEL_TEXTURE_BUFFER_BYTES).zero();
        putTextures(darkened, textures, buffer);
        return buffer;
    }

    static void putTextures(boolean darkened, ForgeOriginalVoxyColourDepthTextureData[] textures, MemoryBuffer into) {
        if (MODEL_TEXTURE_SIZE > 16) {
            throw new IllegalStateException("original MipGen port assumes 16 or smaller model texture size");
        }
