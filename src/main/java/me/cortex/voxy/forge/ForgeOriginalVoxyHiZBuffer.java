...
final class ForgeOriginalVoxyHiZBuffer {
    ...
    private int width;
    private int height;
    private long buildMipChainCount;
    private String lastLifecycleEvent = "created";
    private String lastFailureReason = "none";

    ForgeOriginalVoxyHiZBuffer(ForgeOriginalVoxyRenderProperties properties) {
        this(properties, GL_DEPTH24_STENCIL8);
    }

    ForgeOriginalVoxyHiZBuffer(ForgeOriginalVoxyRenderProperties properties, int type) {
        this.properties = properties;
        this.type = type;
        glNamedFramebufferDrawBuffer(this.framebufferId, GL_NONE);
        this.programId = compileProgram(
                properties.injectDefines(ForgeOriginalVoxyShaderSource.parse("voxy:hiz/blit.vsh")),
                properties.injectDefines(ForgeOriginalVoxyShaderSource.parse("voxy:hiz/blit.fsh")));
    }
    ...
    void buildMipChain(int sourceDepthTextureId, int sourceWidth, int sourceHeight) {
        ...
        glBindVertexArray(0);
        glUseProgram(0);
        this.buildMipChainCount++;
        this.lastLifecycleEvent = "build-mip-chain";
    ...
    boolean ownerReady() {
        return this.framebufferId != 0 && this.samplerId != 0 && this.vertexArrayId != 0 && this.programId != 0;
    }
    ...
    long buildMipChainCount() {
        return this.buildMipChainCount;
    }
    ...
    private void alloc(int width, int height) {
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            ...
            this.lastFailureReason = "hiz-framebuffer-incomplete-" + status;
