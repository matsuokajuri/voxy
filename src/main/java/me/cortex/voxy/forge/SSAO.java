...
final class SSAO extends TrackedObject {
    enum SSAOMode {
        AUTO,
        BASIC,
        BETTER,
    ...
    static SSAO createSSAO(RenderProperties properties, SSAOMode mode) {
        if (mode == SSAOMode.BASIC) {
            return new SSAO(properties, false, 0);
            ...
            if (capabilities.canQueryGpuMemory) {
                ...
                return createSSAO(properties, SSAOMode.BEST);
    ...
    private SSAO(RenderProperties properties, boolean betterSsao, int samples) {
    ...
    void addDebugInfo(List<String> debugLines) {
        debugLines.add("SSAO: " + (this.betterSsao ? "new (" + this.samples + " spp)" : "basic"));
    }
