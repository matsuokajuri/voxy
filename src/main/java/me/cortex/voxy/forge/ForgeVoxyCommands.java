...
public final class ForgeVoxyCommands {
    ...
    static int formalCmdgenGpuValidateBuild(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K5 formal cmdgen GPU validation build: "
                + formatFormalCmdgenGpuValidationStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " Generated commands are validation-only and were not submitted to any draw call."), false);
        return status.cmdgenValidationProgramReady()
    ...
    static int formalCmdgenGpuValidateAudit(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K5 formal cmdgen GPU validation audit: "
                + formatFormalCmdgenGpuValidationAudit(audit)
                + " "
                + formatFormalCmdgenGpuValidationStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)), false);
    ...
    static int formalCmdgenGpuValidateClear(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K5 formal cmdgen GPU validation clear: "
                + formatFormalCmdgenGpuValidationStatus(status)
                + " Only K5 validation program/buffers were cleared; K1-K4 owners, debug command buffers, debug renderers, GL geometry heap, previews, and formal model resources were left unchanged."), false);
        return 1;
    }
    ...
    static int qaK5FormalCmdgenGpuValidation(CommandSourceStack source) {
        ...
        String message = "Voxy QA K5 formal command-generation GPU validation no-draw: "
                + formatFormalCmdgenGpuValidationStatus(status)
                + " "
                + formatFormalCmdgenGpuValidationAudit(audit)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " The QA path used isolated validation-only buffers and did not submit generated commands to glMultiDrawElementsIndirectCountARB, MDICSectionRenderer, VoxyRenderSystem, live terrain draw, visible LoD draw, or debug renderer replacement.";
        VoxyForge.LOGGER.info(message);
        source.sendSuccess(() -> Component.literal(message), false);
        return status.formalTerrainRendererOwnerReady()
                ...
                && status.cmdgenValidationProgramCompileOk()
                && status.cmdgenValidationProgramLinkOk()
                && status.cmdgenValidationDispatchRun()
                && status.cmdgenValidationReadbackOk()
                && status.cmdgenValidationAuditOk()
    ...
    static int formalIsolatedMdicDrawBuild(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K7 formal isolated MDIC draw build: "
                + formatFormalIsolatedMdicDrawStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " K7 draw, if executed, is offscreen validation only and never enables the live renderer."), false);
        return status.isolatedMdicDrawSmokeTestReady()
                && status.offscreenValidationDrawExecuted()
                && status.offscreenReadbackOk()
                && status.offscreenNonZeroPixelCount() > 0
                && status.realSectionCommandUsed()
    ...
    static int formalIsolatedMdicDrawAudit(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K7 formal isolated MDIC draw audit: "
                + formatFormalIsolatedMdicDrawAudit(audit)
                + " "
                + formatFormalIsolatedMdicDrawStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)), false);
    ...
    static int formalIsolatedMdicDrawClear(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K7 formal isolated MDIC draw clear: "
                + formatFormalIsolatedMdicDrawStatus(status)
                + " Only K7 offscreen framebuffer, validation shader, and isolated validation buffers were cleared; K1-K6 owners, debug command buffers, debug renderers, GL geometry heap, and formal model resources were left unchanged."), false);
        return 1;
    }
    ...
    static int qaK7FormalIsolatedMdicDraw(CommandSourceStack source) {
        ...
        ForgeFormalIsolatedMdicDrawSmokeTestStats status = ForgeVoxyInstance.INSTANCE.getFormalIsolatedMdicDrawSmokeTest().build();
        ForgeFormalIsolatedMdicDrawSmokeTestAuditResult audit = ForgeVoxyInstance.INSTANCE.getFormalIsolatedMdicDrawSmokeTest().createAuditStatusSnapshot();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("qa-k7-formal-isolated-mdic-draw");
        String message = "Voxy QA K7 formal isolated MDIC draw smoke test offscreen: "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K7 success requires a real-section K6 command and should be treated as partial if ready=false. ")
                + formatFormalIsolatedMdicDrawStatus(status)
                + " "
                + formatFormalIsolatedMdicDrawAudit(audit)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " The QA path executes only a K7-owned offscreen validation draw from K6 real-section command data; it does not draw into the Minecraft main framebuffer, call MDICSectionRenderer, call VoxyRenderSystem, enable live terrain draw, or replace debug renderers.";
        VoxyForge.LOGGER.info(message);
        source.sendSuccess(() -> Component.literal(message), false);
        return status.formalTerrainRendererOwnerReady()
                && status.formalViewportOwnerReady()
                ...
                && status.offscreenReadbackOk()
                && status.offscreenNonZeroPixelCount() > 0
                && "realSectionCmdgenDryRun".equals(status.drawInputSource())
                && status.realSectionCommandUsed()
                && !status.syntheticDrawFixtureUsed()
                ...
                && status.p0BlockerCount() >= 1
                && audit.success()
                && rendererStatus.formalIsolatedMdicDrawSmokeTestReady()
                && rendererStatus.offscreenValidationDrawExecuted()
                && !rendererStatus.formalRendererReady()
                && !rendererStatus.actualDrawEnabled() ? 1 : 0;
    }
    ...
    static int formalModelIdGeometryPathBuild(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K8 formal model-id section geometry path build: "
                + formatFormalModelIdGeometryPathStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " K8 creates only an isolated formal geometry snapshot/validation buffer; it does not mutate the original GL geometry heap or enable live draw."), false);
        return status.formalModelIdGeometryPathReady()
                && status.globalFormalModelIdGeometryPathReady()
                && status.formalGeometrySnapshotCreated()
                && status.formalGeometrySnapshotRecordCount() >= 1
                && status.usesFormalModelIds()
                && !status.usesPlaceholderModelIds()
                && !status.sampleSetModelIdsUsed()
                && !status.originalGeometryHeapMutated()
    ...
    static int formalModelIdGeometryPathAudit(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K8 formal model-id section geometry path audit: "
                + formatFormalModelIdGeometryPathAudit(audit)
                + " "
                + formatFormalModelIdGeometryPathStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)), false);
    ...
    static int formalModelIdGeometryPathClear(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K8 formal model-id section geometry path clear: "
                + formatFormalModelIdGeometryPathStatus(status)
                + " Only K8 isolated formal geometry snapshot state and validation buffer were cleared; debug renderers, live GL geometry heap, formal ModelStore, K6/K7 resources, and MDIC command buffers were left unchanged."), false);
        return 1;
    }
    ...
    static int qaK8FormalModelIdGeometryPath(CommandSourceStack source) {
        ...
        boolean engineReady = ForgeVoxyInstance.INSTANCE.ensureActiveWorldSkeletonForCurrentWorldIfAllowed();
        ForgeFormalModelIdSectionGeometryStats status = ForgeVoxyInstance.INSTANCE.getFormalModelIdSectionGeometryPath().build();
        ForgeFormalModelIdSectionGeometryAuditResult audit = ForgeVoxyInstance.INSTANCE.getFormalModelIdSectionGeometryPath().createAuditStatusSnapshot();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("qa-k8-formal-model-id-geometry-path");
        String message = "Voxy QA K8 formal model-id section geometry path no live draw: "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K8 success requires real section input and should be treated as partial if ready=false. ")
                + formatFormalModelIdGeometryPathStatus(status)
                + " "
                + formatFormalModelIdGeometryPathAudit(audit)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " The QA path builds a K8-owned isolated formal geometry snapshot with formal model ids, verifies readback, and does not mutate the original GL geometry heap, submit live draw, call MDICSectionRenderer, or call VoxyRenderSystem.";
        VoxyForge.LOGGER.info(message);
        source.sendSuccess(() -> Component.literal(message), false);
        return status.formalTerrainRendererOwnerReady()
                && status.formalModelStoreOwnerReady()
                ...
                && status.realTerrainPackedRecordBridgeReady()
                && status.realSectionInputUsed()
                && status.formalModelIdGeometryPathReady()
                && status.globalFormalModelIdGeometryPathReady()
                && !status.globalFormalModelIdGeometryEnabledForLiveRenderer()
                && status.formalGeometrySnapshotCreated()
                && status.formalGeometrySnapshotRecordCount() >= 1
                && status.acceptedSectionCount() >= 1
                && status.acceptedRecordCount() >= 1
                && status.usesFormalModelIds()
                && !status.usesPlaceholderModelIds()
                && !status.sampleSetModelIdsUsed()
                ...
                && status.p0BlockerCount() >= 1
                && audit.success()
                && rendererStatus.globalFormalModelIdGeometryPathReady()
                && !rendererStatus.globalFormalModelIdGeometryEnabledForLiveRenderer()
                && !rendererStatus.formalRendererReady()
                && !rendererStatus.actualDrawEnabled() ? 1 : 0;
    }
    ...
    static int formalVisibleLodPreviewEnable(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10 formal visible LoD preview enable: "
                ...
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " Preview draw is explicit opt-in only and remains preview-only; formalRendererReady=false actualRendererDrawEnabled=false."), false);
        return status.visibleLodPreviewOwnerReady()
                && status.visiblePreviewEnabled()
                && status.debugOptInOnly()
                && !status.productionLiveRendererDrawExecuted()
                && !status.formalRendererReady()
                && !status.actualRendererDrawEnabled() ? 1 : 0;
    }
    ...
    static int formalVisibleLodPreviewDisable(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10 formal visible LoD preview disable: "
                ...
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " The visible preview hook remains registered but drawing is disabled."), false);
        return !status.visiblePreviewEnabled() && !status.actualRendererDrawEnabled() ? 1 : 0;
    }
    ...
    static int formalVisibleLodPreviewPrepare(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K11 visible LoD preview prepare: "
                ...
                + " prepareRequested=" + status.observePrepareRequested()
                + " prepareInProgress=" + status.observePrepareInProgress()
                + " prepareCompleted=" + status.observePrepareCompleted()
                + " commandReturnedQuickly=" + status.observeEnableCommandReturnedQuickly()
                + " commandDurationMs=" + status.observeEnableCommandDurationMillis()
                + " visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " formalRendererReady=false actualRendererDrawEnabled=false"
                + " hint=wait-then-run-prepare_status"), false);
        return (previewPrepared || status.observePrepareRequested())
    ...
    static int formalVisibleLodPreviewPrepareStatus(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K11 visible LoD preview prepare status: "
                ...
                + formatFormalVisibleLodPreviewStatus(status)), false);
    ...
    static int formalVisibleLodPreviewObserveEnable(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10.3 observe enable requested: "
                + "k11PreviewPrepared=" + previewPrepared
                + " commandReturnedQuickly=" + status.observeEnableCommandReturnedQuickly()
                + " commandDurationMs=" + status.observeEnableCommandDurationMillis()
                + " glOnCommandThread=" + status.observeEnableDidGlWorkOnCommandThread()
                + " readbackOnCommandThread=" + status.observeEnableDidReadbackOnCommandThread()
                + " synchronousRebuild=" + status.observeEnableDidSynchronousRebuild()
                + " prepareRequested=" + status.observePrepareRequested()
                + " visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " lastFailureReason=" + status.lastFailureReason()
                + " formalRendererReady=false actualRendererDrawEnabled=false"
                + (previewPrepared ? " hint=status-after-green-block" : " hint=run-formal_visible_lod_preview_prepare-first")), false);
    ...
    static int formalVisibleLodPreviewObserveDisable(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10.3 observe disable: "
                + "visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " observeModeEnabled=" + status.observeModeEnabled()
                + " renderHookEarlyReturnWhenDisabled=" + status.renderHookEarlyReturnWhenDisabled()
                + " formalRendererReady=false actualRendererDrawEnabled=false"
                + " diagnostic=draw-disabled-resources-kept"), false);
    ...
    static int formalVisibleLodPreviewObserveDrawPause(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10 visible preview observe draw pause: observeDrawPaused="
                + preview.isObserveDrawPaused()
                + " visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " visiblePreviewDrawExecuted=" + status.visiblePreviewDrawExecuted()
                + " formalRendererReady=" + status.formalRendererReady()
                + " actualRendererDrawEnabled=" + status.actualRendererDrawEnabled()
                + " diagnostic=draw-call-suppressed-resources-kept"), false);
    ...
    static int formalVisibleLodPreviewObserveDrawResume(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10 visible preview observe draw resume: observeDrawPaused="
                ...
                + " formalRendererReady=" + status.formalRendererReady()
                + " actualRendererDrawEnabled=" + status.actualRendererDrawEnabled()
                + " diagnostic=draw-call-restored"), false);
        return !preview.isObserveDrawPaused() && !status.formalRendererReady() && !status.actualRendererDrawEnabled() ? 1 : 0;
    }
    ...
    static int formalVisibleLodPreviewAudit(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10 formal visible LoD preview audit: "
    ...
    static int formalVisibleLodPreviewClear(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.literal("Voxy K10 formal visible LoD preview clear: "
                + formatFormalVisibleLodPreviewStatus(status)
                + " Only K10 visible-preview shader and validation buffers were cleared; K8 geometry, formal ModelStore, debug renderers, live GL geometry heap, and MDIC command buffers were left unchanged."), false);
        return 1;
    }
    ...
    static int qaK10FormalVisibleLodPreview(CommandSourceStack source) {
        ...
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("qa-k10-formal-visible-lod-preview");
        String message = "Voxy QA K10 formal visible LoD preview: "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K10 success requires real section input and should be treated as partial if ready=false. ")
                + "The preview is disabled by default, is now explicitly enabled for a short QA window, and will auto-disable after visible frames. "
                + formatFormalVisibleLodPreviewStatus(status)
                + " "
                + formatFormalVisibleLodPreviewAudit(audit)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " The QA path uses K8 formal model-id geometry, K9 terrain shader integration, and K6 real-section command evidence; it does not call MDICSectionRenderer or VoxyRenderSystem and does not enable the production renderer.";
        VoxyForge.LOGGER.info(message);
        source.sendSuccess(() -> Component.literal(message), false);
        return status.formalTerrainRendererOwnerReady()
                && status.formalModelIdGeometryPathReady()
                && status.formalTerrainShaderIntegrationReady()
                && status.visibleLodPreviewOwnerReady()
                && status.visiblePreviewDefaultDisabledVerified()
                && status.debugOptInOnly()
                && status.visiblePreviewEnabled()
                && status.visiblePreviewWasEnabledDuringQa()
                && status.k8FormalGeometryUsed()
    ...
    static int qaK10VisiblePreviewObservePrepare(CommandSourceStack source) {
        ...
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("qa-k10-visible-preview-observe-prepare");
        String message = "Voxy QA K10.3 visible preview observe prepare: "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; observe prepare can be requested but drawing cannot be proven. ")
                + "Compatibility alias for K11 prepare: command path returns quickly, does no command-thread GL/readback/synchronous rebuild, and the render hook prepares K6-K10 resources without enabling observe draw. "
                + formatFormalVisibleLodPreviewStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " Rerun /voxy formal_visible_lod_preview_prepare_status after a few frames; expected success has observePrepareCompleted=true observeAutoEnabledAfterPrepare=false visiblePreviewEnabled=false.";
        VoxyForge.LOGGER.info(message);
        source.sendSuccess(() -> Component.literal(message), false);
        return status.observePrepareRequested()
                && status.observeEnableCommandReturnedQuickly()
                && !status.observeEnableDidGlWorkOnCommandThread()
                && !status.observeEnableDidReadbackOnCommandThread()
                && !status.observeEnableDidSynchronousRebuild()
    ...
    static int qaK11FormalVisibleLodPreviewPrewarm(CommandSourceStack source) {
        ...
        boolean previewPrepared = status.visibleLodPreviewOwnerReady() && !status.stale();
        source.sendSuccess(() -> Component.literal("Voxy QA K11 formal visible LoD preview prewarm: "
                ...
                + " visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " formalRendererReady=false actualRendererDrawEnabled=false"
                + " next=/voxy formal_visible_lod_preview_prepare_status"), false);
        return (previewPrepared || status.observePrepareRequested())
                && status.observeEnableCommandReturnedQuickly()
                && !status.observeEnableDidGlWorkOnCommandThread()
                && !status.observeEnableDidReadbackOnCommandThread()
    ...
    static int qaK12K13VisiblePreviewPrepareReuse(CommandSourceStack source) {
        ...
        boolean previewPrepared = status.visibleLodPreviewOwnerReady() && !status.stale();
        source.sendSuccess(() -> Component.literal("Voxy QA K12/K13 visible preview prepare/reuse: "
                + (engineReady ? "WorldEngine active. " : "No active client world; prepare may wait/fail safely. ")
                + "previewPrepared=" + previewPrepared
                + " prepareRequested=" + status.observePrepareRequested()
                + " prepareInProgress=" + status.observePrepareInProgress()
                + " prepareCompleted=" + status.observePrepareCompleted()
                + " prepareTimingReady=" + status.prepareTimingReady()
                + " prepareReuseReady=" + status.prepareReuseReady()
                + " lastPrepareReusedExistingResources=" + status.lastPrepareReusedExistingResources()
                + " unnecessaryRebuildDetected=" + status.unnecessaryRebuildDetected()
                + " commandReturnedQuickly=" + status.observeEnableCommandReturnedQuickly()
                + " commandDurationMs=" + status.observeEnableCommandDurationMillis()
                + " visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " formalRendererReady=false actualRendererDrawEnabled=false"
        ...
        return (previewPrepared || status.observePrepareRequested() || status.observePrepareInProgress())
                ...
                && !status.observeEnableDidReadbackOnCommandThread()
                && !status.observeEnableDidSynchronousRebuild()
                && !status.unnecessaryRebuildDetected()
                && !status.formalRendererReady()
                && !status.actualRendererDrawEnabled() ? 1 : 0;
    }
    ...
    static int formalMinimalLodRendererPrepare(CommandSourceStack source) {
        ...
        boolean engineReady = ForgeVoxyInstance.INSTANCE.ensureActiveWorldSkeletonForCurrentWorldIfAllowed();
        ForgeFormalVisibleLodPreviewStats status = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview()
                .requestMinimalFormalLodPatchPrepare("k23-k30-minimal-formal-lod-renderer-prepare");
        source.sendSuccess(() -> Component.literal("Voxy K23-K30 minimal formal LoD renderer prepare: "
                + (engineReady ? "WorldEngine active. " : "No active client world; prepare may wait/fail safely. ")
                + formatMinimalFormalLodRendererStatus(status)
                + " next=/voxy formal_minimal_lod_renderer_status; enable=/voxy formal_minimal_lod_renderer_enable"), false);
        return (minimalFormalLodRendererPrototypeReady(status)
    ...
    static int formalMinimalLodRendererStatus(CommandSourceStack source) {
        ...
        return minimalFormalLodRendererPrototypeReady(status)
                && boundedFormalLodPatchReady(status)
                && !status.productionMdicIndirectDrawUsed()
                && !status.formalRendererReady()
                && !status.actualRendererDrawEnabled() ? 1 : 0;
    }
    ...
    static int qaK23K30MinimalFormalLodRenderer(CommandSourceStack source) {
        ...
        boolean engineReady = ForgeVoxyInstance.INSTANCE.ensureActiveWorldSkeletonForCurrentWorldIfAllowed();
        ForgeFormalVisibleLodPreviewStats status = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview()
                .requestMinimalFormalLodPatchPrepare("qa-k23-k30-minimal-formal-lod-renderer");
        boolean previewPrepared = status.visibleLodPreviewOwnerReady() && !status.stale();
        source.sendSuccess(() -> Component.literal("Voxy QA K23-K30 minimal formal LoD renderer: "
                + (engineReady ? "WorldEngine active. " : "No active client world; prepare may wait/fail safely. ")
                + formatMinimalFormalLodRendererStatus(status)
                + " next=/voxy formal_minimal_lod_renderer_status; enable=/voxy formal_minimal_lod_renderer_enable; disable=/voxy formal_minimal_lod_renderer_disable"), false);
        return (previewPrepared || status.observePrepareRequested() || status.observePrepareInProgress())
                && !status.productionMdicIndirectDrawUsed()
                && !status.formalRendererReady()
                && !status.actualRendererDrawEnabled() ? 1 : 0;
    ...
    static int formalLodPreviewMovingPrepare(CommandSourceStack source) {
        ...
        boolean engineReady = ForgeVoxyInstance.INSTANCE.ensureActiveWorldSkeletonForCurrentWorldIfAllowed();
        ForgeFormalVisibleLodPreviewStats status = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview()
                .requestMovingFormalLodPatchPrepare("k31-k36-formal-lod-preview-moving-prepare");
        source.sendSuccess(() -> Component.literal("Voxy K31-K36 formal LoD moving preview prepare: "
                + (engineReady ? "WorldEngine active. " : "No active client world; prepare may wait/fail safely. ")
                + formatFormalLodPreviewMovingStatus(status)
                + " next=/voxy formal_lod_preview_moving_status; enable=/voxy formal_lod_preview_moving_enable"), false);
        return (movingFormalLodPreviewReady(status)
                || status.observePrepareRequested()
                || status.observePrepareInProgress())
                && !status.formalRendererReady()
                && !status.actualRendererDrawEnabled() ? 1 : 0;
    ...
    static int formalLodPreviewMovingEnable(CommandSourceStack source) {
        ...
        boolean previewPrepared = status.visibleLodPreviewOwnerReady() && !status.stale();
        source.sendSuccess(() -> Component.literal("Voxy K31-K36 formal LoD moving preview enable: "
                + "previewPrepared=" + previewPrepared
                + " commandReturnedQuickly=" + status.observeEnableCommandReturnedQuickly()
                + " visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " " + formatFormalLodPreviewMovingStatus(status)
                + (previewPrepared ? " hint=wait-a-few-frames-then-status" : " hint=run-formal_lod_preview_moving_prepare-first")), false);
        return (status.observeEnableRequested() || status.visiblePreviewEnabled())
                && status.observeEnableCommandReturnedQuickly()
                && !status.observeEnableDidGlWorkOnCommandThread()
                && !status.observeEnableDidReadbackOnCommandThread()
                && !status.observeEnableDidSynchronousRebuild()
    ...
    static int qaK31K36FormalLodPreviewMovingPatch(CommandSourceStack source) {
        ...
        boolean engineReady = ForgeVoxyInstance.INSTANCE.ensureActiveWorldSkeletonForCurrentWorldIfAllowed();
        ForgeFormalVisibleLodPreviewStats status = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview()
                .requestMovingFormalLodPatchPrepare("qa-k31-k36-formal-lod-preview-moving-patch");
        boolean previewPrepared = status.visibleLodPreviewOwnerReady() && !status.stale();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager()
                .checkReadiness("qa-k31-k36-formal-lod-preview-moving-patch");
        String message = "Voxy QA K31-K36 formal LoD moving preview patch: "
                + (engineReady ? "WorldEngine active. " : "No active client world; prepare may wait/fail safely. ")
                + formatFormalLodPreviewMovingStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " next=/voxy formal_lod_preview_moving_status; enable=/voxy formal_lod_preview_moving_enable; disable=/voxy formal_lod_preview_moving_disable";
        VoxyForge.LOGGER.info(message);
        source.sendSuccess(() -> Component.literal(message), false);
        return (previewPrepared || status.observePrepareRequested() || status.observePrepareInProgress())
                && status.observeEnableCommandReturnedQuickly()
    ...
    static int formalLodPreviewExpandedPrepare(CommandSourceStack source) {
        ...
        boolean engineReady = ForgeVoxyInstance.INSTANCE.ensureActiveWorldSkeletonForCurrentWorldIfAllowed();
        ForgeFormalVisibleLodPreviewStats status = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview()
                .requestExpandedFormalLodPatchPrepare("k37-k42-expanded-formal-lod-preview-prepare");
        source.sendSuccess(() -> Component.literal("Voxy K37-K42 expanded formal LoD patch prepare: "
                + (engineReady ? "WorldEngine active. " : "No active client world; prepare may wait/fail safely. ")
                + formatFormalLodPreviewExpandedStatus(status)
                + " next=/voxy formal_lod_preview_expanded_status; enable=/voxy formal_lod_preview_expanded_enable"), false);
        return (expandedFormalLodPatchReady(status)
                || status.observePrepareRequested()
                || status.observePrepareInProgress())
                && !status.formalRendererReady()
                && !status.actualRendererDrawEnabled() ? 1 : 0;
    ...
    static int formalLodPreviewExpandedEnable(CommandSourceStack source) {
        ...
        boolean previewPrepared = status.visibleLodPreviewOwnerReady() && !status.stale();
        source.sendSuccess(() -> Component.literal("Voxy K37-K42 expanded formal LoD patch enable: "
                + "previewPrepared=" + previewPrepared
                + " commandReturnedQuickly=" + status.observeEnableCommandReturnedQuickly()
                + " visiblePreviewEnabled=" + status.visiblePreviewEnabled()
                + " " + formatFormalLodPreviewExpandedStatus(status)
                + (previewPrepared ? " hint=wait-a-few-frames-then-status" : " hint=run-formal_lod_preview_expanded_prepare-first")), false);
        return (status.observeEnableRequested() || status.visiblePreviewEnabled())
                && status.observeEnableCommandReturnedQuickly()
                && !status.observeEnableDidGlWorkOnCommandThread()
                && !status.observeEnableDidReadbackOnCommandThread()
    ...
    static int qaK37K42ExpandedFormalLodPatch(CommandSourceStack source) {
        ...
        boolean engineReady = ForgeVoxyInstance.INSTANCE.ensureActiveWorldSkeletonForCurrentWorldIfAllowed();
        ForgeFormalVisibleLodPreviewStats status = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview()
                .requestExpandedFormalLodPatchPrepare("qa-k37-k42-expanded-formal-lod-patch");
        boolean previewPrepared = status.visibleLodPreviewOwnerReady() && !status.stale();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager()
                .checkReadiness("qa-k37-k42-expanded-formal-lod-patch");
        String message = "Voxy QA K37-K42 expanded formal LoD patch: "
                + (engineReady ? "WorldEngine active. " : "No active client world; prepare may wait/fail safely. ")
                + formatFormalLodPreviewExpandedStatus(status)
                + " "
                + formatFormalRendererStatus(rendererStatus)
                + " next=/voxy formal_lod_preview_expanded_status; enable=/voxy formal_lod_preview_expanded_enable; disable=/voxy formal_lod_preview_expanded_disable";
        VoxyForge.LOGGER.info(message);
        source.sendSuccess(() -> Component.literal(message), false);
        return (previewPrepared || status.observePrepareRequested() || status.observePrepareInProgress())
                && status.observeEnableCommandReturnedQuickly()
    ...
    static String formatFormalTerrainRendererOwnerStatus(ForgeFormalTerrainRendererStats status) {
        return String.format(
                ...
                status.formalHierarchicalOcclusionReady(),
                status.formalRenderDistanceTrackerReady(),
                status.cpuCandidateSnapshotReady(),
    ...
    static String formatFormalTerrainRendererOwnerAudit(ForgeFormalTerrainRendererAuditResult audit) {
        return String.format(
    ...
    static String formatFormalMdicViewportOwnerStatus(ForgeFormalMdicViewportStats status) {
        return String.format(
    ...
    static String formatFormalMdicViewportOwnerAudit(ForgeFormalMdicViewportAuditResult audit) {
        return String.format(
    ...
    static String formatFormalCommandGenerationOwnerStatus(ForgeFormalCommandGenerationStats status) {
        return String.format(
                "stage=%s formalTerrainRendererOwnerReady=%s formalViewportOwnerReady=%s formalCommandBufferOwnerReady=%s formalCommandGenerationOwnerReady=%s formalCommandGenerationContractReady=%s formalDrawCommandLayoutReady=%s formalDrawCountLayoutReady=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s noDraw=%s enabled=%s originalVoxyCmdgenAlignmentChecked=%s originalVoxyBindingsAlignmentChecked=%s originalVoxyMdicViewportAlignmentChecked=%s originalVoxyMdicSectionRendererAlignmentChecked=%s debugMdicCommandBuffersUsedAsFormal=%s debugRendererIsolationOk=%s drawCommandStructKnown=%s drawCommandStrideBytes=%d drawCommandFieldCount=%d drawCommandIndexedIndirectCompatible=%s drawCountBufferLayoutKnown=%s drawCountBufferStrideBytes=%d drawCountBufferHeaderWords=%d drawCountBufferLayoutBytes=%d layoutCompatibility=%s layoutMismatchReason=%s visibilityInputRequired=%s renderListInputRequired=%s indirectLookupInputRequired=%s sectionMetadataInputRequired=%s positionScratchInputRequired=%s geometryBufferInputRequired=%s modelStoreInputRequired=%s commandGenerationInputsKnown=%s commandGenerationOutputsKnown=%s commandGenerationGpuProgramReady=%s cmdgenComputeShaderRun=%s glDispatchComputeIndirectCalled=%s glMultiDrawElementsIndirectCountCalled=%s mdicSectionRendererCalled=%s voxyRenderSystemCalled=%s visibilityTraversalReady=%s globalFormalModelIdGeometryReady=%s formalTerrainShaderReady=%s drawBufferBinding=%d drawCountBufferBinding=%d sectionMetadataBufferBinding=%d visibilityBufferBinding=%d indirectLookupBufferBinding=%d positionScratchBufferBinding=%d translucentDistanceBufferBinding=%d logicalOnly=%s allocated=%s allocationDeferredReason=%s lifecycleState=%s lastLifecycleEvent=%s lifecycleGeneration=%d stale=%s requiresRebuild=%s blockerCount=%d p0BlockerCount=%d p1BlockerCount=%d p2BlockerCount=%d blockers=%s resourceReloadSeen=%s worldUnloadSeen=%s dimensionSwitchSeen=%s debugPipelineClearSeen=%s presetOffSeen=%s presetClearSeen=%s lastFailureReason=%s lastAuditOk=%s lastAuditError=%s renderer=formal-command-generation-owner formalRenderer=false draw=false",
                status.stage(),
                status.formalTerrainRendererOwnerReady(),
                status.formalViewportOwnerReady(),
                status.formalCommandBufferOwnerReady(),
                ...
                status.debugMdicCommandBuffersUsedAsFormal(),
                status.debugRendererIsolationOk(),
                status.drawCommandStructKnown(),
                status.drawCommandStrideBytes(),
                status.drawCommandFieldCount(),
                status.drawCommandIndexedIndirectCompatible(),
                status.drawCountBufferLayoutKnown(),
                status.drawCountBufferStrideBytes(),
                status.drawCountBufferHeaderWords(),
                status.drawCountBufferLayoutBytes(),
                status.layoutCompatibility(),
                status.layoutMismatchReason(),
                status.visibilityInputRequired(),
                status.renderListInputRequired(),
                status.indirectLookupInputRequired(),
                status.sectionMetadataInputRequired(),
                status.positionScratchInputRequired(),
                status.geometryBufferInputRequired(),
                status.modelStoreInputRequired(),
                status.commandGenerationInputsKnown(),
                status.commandGenerationOutputsKnown(),
                status.commandGenerationGpuProgramReady(),
                status.cmdgenComputeShaderRun(),
                status.glDispatchComputeIndirectCalled(),
                status.glMultiDrawElementsIndirectCountCalled(),
                status.mdicSectionRendererCalled(),
                status.voxyRenderSystemCalled(),
                status.visibilityTraversalReady(),
                status.globalFormalModelIdGeometryReady(),
                status.formalTerrainShaderReady(),
                status.drawBufferBinding(),
                status.drawCountBufferBinding(),
                status.sectionMetadataBufferBinding(),
                status.visibilityBufferBinding(),
                status.indirectLookupBufferBinding(),
                status.positionScratchBufferBinding(),
                status.translucentDistanceBufferBinding(),
                status.logicalOnly(),
                status.allocated(),
                status.allocationDeferredReason(),
                status.lifecycleState(),
    ...
    static String formatFormalCommandGenerationOwnerAudit(ForgeFormalCommandGenerationAuditResult audit) {
        return String.format(
                "k3AuditOk=%s k3AuditError=%s k1OwnerExists=%s k2ViewportOwnerExists=%s formalCommandGenerationOwnerExists=%s cmdgenCompInspected=%s bindingsGlslInspected=%s mdicViewportInspected=%s mdicSectionRendererInspected=%s debugMdicCommandBuffersUsedAsFormal=%s commandInputContractExplicit=%s commandOutputContractExplicit=%s drawCommandLayoutReady=%s drawCountLayoutReady=%s cmdgenComputeShaderRun=%s multiDrawIndirectCountCalled=%s mdicSectionRendererCalled=%s voxyRenderSystemCalled=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s",
                audit.success(),
                audit.error(),
                audit.k1OwnerExists(),
                audit.k2ViewportOwnerExists(),
                audit.formalCommandGenerationOwnerExists(),
                audit.cmdgenCompInspected(),
                audit.bindingsGlslInspected(),
                audit.mdicViewportInspected(),
                audit.mdicSectionRendererInspected(),
                audit.debugMdicCommandBuffersUsedAsFormal(),
                audit.commandInputContractExplicit(),
                audit.commandOutputContractExplicit(),
                audit.drawCommandLayoutReady(),
                audit.drawCountLayoutReady(),
                audit.cmdgenComputeShaderRun(),
                audit.multiDrawIndirectCountCalled(),
                audit.mdicSectionRendererCalled(),
                audit.voxyRenderSystemCalled(),
    ...
    static String formatFormalVisibilityOwnerStatus(ForgeFormalVisibilityStats status) {
        return String.format(
                "stage=%s formalTerrainRendererOwnerReady=%s formalViewportOwnerReady=%s formalCommandGenerationOwnerReady=%s formalVisibilityOwnerReady=%s formalRenderListOwnerReady=%s formalIndirectLookupOwnerReady=%s formalVisibilityContractReady=%s formalRenderListContractReady=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s noDraw=%s enabled=%s originalVoxyHierarchicalOcclusionAlignmentChecked=%s originalVoxyRenderDistanceTrackerAlignmentChecked=%s originalVoxyViewportAlignmentChecked=%s originalVoxyMdicViewportAlignmentChecked=%s originalVoxyCmdgenVisibilityAlignmentChecked=%s formalVisibilityTraversalImplemented=%s formalHierarchicalOcclusionReady=%s formalRenderDistanceTrackerReady=%s cpuCandidateSnapshotReady=%s cpuCandidateSnapshotUsed=%s candidateSectionCount=%d candidateSectionSource=%s candidateSectionIdsKnown=%s candidateSectionPositionsKnown=%s visibilityBufferContractKnown=%s renderListContractKnown=%s indirectLookupContractKnown=%s formalVisibilityBufferContractReady=%s formalRenderListOrIndirectLookupContractReady=%s debugPlannerUsedAsFormal=%s debugPlannerComparedOnly=%s cmdgenComputeShaderRun=%s glMultiDrawElementsIndirectCountCalled=%s mdicSectionRendererCalled=%s voxyRenderSystemCalled=%s logicalOnly=%s allocated=%s allocationDeferredReason=%s lifecycleState=%s lastLifecycleEvent=%s lifecycleGeneration=%d stale=%s requiresRebuild=%s blockerCount=%d p0BlockerCount=%d p1BlockerCount=%d p2BlockerCount=%d blockers=%s resourceReloadSeen=%s worldUnloadSeen=%s dimensionSwitchSeen=%s debugPipelineClearSeen=%s presetOffSeen=%s presetClearSeen=%s lastFailureReason=%s lastAuditOk=%s lastAuditError=%s renderer=formal-visibility-owner formalRenderer=false draw=false",
                status.stage(),
                status.formalTerrainRendererOwnerReady(),
                status.formalViewportOwnerReady(),
                status.formalCommandGenerationOwnerReady(),
                ...
                status.candidateSectionIdsKnown(),
                status.candidateSectionPositionsKnown(),
                status.visibilityBufferContractKnown(),
                status.renderListContractKnown(),
                status.indirectLookupContractKnown(),
                status.formalVisibilityBufferContractReady(),
                status.formalRenderListOrIndirectLookupContractReady(),
                status.debugPlannerUsedAsFormal(),
                status.debugPlannerComparedOnly(),
                status.cmdgenComputeShaderRun(),
                status.glMultiDrawElementsIndirectCountCalled(),
                status.mdicSectionRendererCalled(),
                status.voxyRenderSystemCalled(),
    ...
    static String formatFormalVisibilityOwnerAudit(ForgeFormalVisibilityAuditResult audit) {
        return String.format(
                "k4AuditOk=%s k4AuditError=%s k1OwnerExists=%s k2ViewportOwnerExists=%s k3CommandGenerationOwnerExists=%s formalVisibilityOwnerExists=%s hierarchicalOcclusionInspected=%s renderDistanceTrackerInspected=%s viewportInspected=%s mdicViewportInspected=%s cmdgenVisibilityContractInspected=%s visibilityContractExplicit=%s renderListContractExplicit=%s debugPlannerUsedAsFormal=%s cpuCandidateSnapshotProvisional=%s cmdgenComputeShaderRun=%s multiDrawIndirectCountCalled=%s mdicSectionRendererCalled=%s voxyRenderSystemCalled=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s",
                audit.success(),
                audit.error(),
                audit.k1OwnerExists(),
                audit.k2ViewportOwnerExists(),
                audit.k3CommandGenerationOwnerExists(),
                audit.formalVisibilityOwnerExists(),
                audit.hierarchicalOcclusionInspected(),
                audit.renderDistanceTrackerInspected(),
                audit.viewportInspected(),
                audit.mdicViewportInspected(),
                audit.cmdgenVisibilityContractInspected(),
                audit.visibilityContractExplicit(),
                audit.renderListContractExplicit(),
                audit.debugPlannerUsedAsFormal(),
                audit.cpuCandidateSnapshotProvisional(),
                audit.cmdgenComputeShaderRun(),
                audit.multiDrawIndirectCountCalled(),
                audit.mdicSectionRendererCalled(),
                audit.voxyRenderSystemCalled(),
    ...
    private static String formatFormalCmdgenGpuValidationStatus(ForgeFormalCmdgenGpuValidationStats status) {
        return String.format(
                "stage=%s formalTerrainRendererOwnerReady=%s formalViewportOwnerReady=%s formalCommandGenerationOwnerReady=%s formalVisibilityOwnerReady=%s cmdgenValidationProgramReady=%s cmdgenValidationProgramCompileAttempted=%s cmdgenValidationProgramCompileOk=%s cmdgenValidationProgramLinkOk=%s cmdgenValidationProgramId=%d cmdgenValidationDispatchRun=%s cmdgenValidationReadbackOk=%s cmdgenValidationAuditOk=%s originalCmdgenFullyOperational=%s productionCmdgenReady=%s validationInputSource=%s syntheticValidationFixtureUsed=%s cpuCandidateSnapshotUsed=%s validationDrawCommandBufferAllocated=%s validationDrawCountBufferAllocated=%s validationVisibilityBufferAllocated=%s validationRenderListBufferAllocated=%s validationPositionScratchBufferAllocated=%s validationSectionMetadataBufferAllocated=%s validationOnly=%s liveRendererBuffer=%s debugBuffer=%s drawCommandStructKnown=%s drawCommandStrideBytes=%d drawCommandFieldCount=%d drawCommandIndexedIndirectCompatible=%s generatedCommandCount=%d generatedDrawCount=%d drawCommandReadbackOk=%s drawCountReadbackOk=%s positionScratchReadbackOk=%s firstCommandCount=%d firstCommandInstanceCount=%d firstCommandFirstIndex=%d firstCommandBaseVertex=%d firstCommandBaseInstance=%d positionScratchWord0=%d positionScratchWord1=%d debugMdicCommandBuffersUsedAsFormal=%s debugRendererIsolationOk=%s glMultiDrawElementsIndirectCountCalled=%s MDICSectionRendererCalled=%s VoxyRenderSystemCalled=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s noDraw=%s lifecycleState=%s lastLifecycleEvent=%s stale=%s requiresRebuild=%s blockerCount=%d p0BlockerCount=%d p1BlockerCount=%d p2BlockerCount=%d blockers=%s lastGlError=%s lastFailureReason=%s lastAuditOk=%s lastAuditError=%s renderer=formal-cmdgen-gpu-validator formalRenderer=false draw=false",
                status.stage(),
                status.formalTerrainRendererOwnerReady(),
                status.formalViewportOwnerReady(),
                status.formalCommandGenerationOwnerReady(),
                status.formalVisibilityOwnerReady(),
                status.cmdgenValidationProgramReady(),
                status.cmdgenValidationProgramCompileAttempted(),
                status.cmdgenValidationProgramCompileOk(),
                status.cmdgenValidationProgramLinkOk(),
                status.cmdgenValidationProgramId(),
                status.cmdgenValidationDispatchRun(),
                status.cmdgenValidationReadbackOk(),
                status.cmdgenValidationAuditOk(),
    ...
    private static String formatFormalCmdgenGpuValidationAudit(ForgeFormalCmdgenGpuValidationAuditResult audit) {
        return String.format(
                "k5AuditOk=%s k5AuditError=%s durationMs=%.2f k1OwnerExists=%s k2ViewportOwnerExists=%s k3CommandGenerationOwnerExists=%s k4VisibilityOwnerExists=%s cmdgenCompInspected=%s bindingsGlslInspected=%s validationBuffersIsolated=%s debugMdicCommandBuffersUsedAsFormal=%s validationDispatchRun=%s liveCommandGenerationRun=%s drawCommandReadbackOk=%s drawCountReadbackOk=%s positionScratchReadbackOk=%s generatedCommandCount=%d generatedDrawCount=%d multiDrawIndirectCountCalled=%s mdicSectionRendererCalled=%s voxyRenderSystemCalled=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s",
                audit.success(),
                audit.error(),
                audit.durationMs(),
                audit.k1OwnerExists(),
    ...
    private static String formatFormalIsolatedMdicDrawStatus(ForgeFormalIsolatedMdicDrawSmokeTestStats status) {
        return String.format(
                "stage=%s formalTerrainRendererOwnerReady=%s formalViewportOwnerReady=%s formalCommandGenerationOwnerReady=%s formalVisibilityOwnerReady=%s realSectionDryRunReady=%s isolatedMdicDrawSmokeTestReady=%s offscreenValidationDrawReady=%s offscreenFramebufferCreated=%s offscreenFramebufferComplete=%s offscreenValidationDrawExecuted=%s offscreenReadbackOk=%s offscreenNonZeroPixelCount=%d offscreenChecksum=%s offscreenSize=%dx%d drawInputSource=%s realSectionCommandUsed=%s syntheticDrawFixtureUsed=%s drawCommandMatchesK6=%s generatedCommandCount=%d acceptedDrawCommandCount=%d drawCountAccepted=%s geometryInputAvailable=%s modelInputAvailable=%s shaderInputAvailable=%s isolatedGeometryBufferUsed=%s originalGeometryHeapMutated=%s liveGeometryHeapUsedAsMutableTarget=%s formalModelIdsUsed=%s placeholderModelIdsUsed=%s sampleSetUsedAsFormalSource=%s validationFormalModelId=%d validationFormalModelIds=%s validationDrawShaderUsed=%s productionTerrainShaderUsed=%s formalTerrainShaderReady=%s shaderContractSubset=%s modelDataBindingOk=%s modelColourBindingOk=%s atlasTextureBindingOk=%s samplerBindingOk=%s bindingLayoutCompatible=%s glMultiDrawElementsIndirectCountCalled=%s glMultiDrawElementsIndirectCountCallScope=%s MDICSectionRendererCalled=%s VoxyRenderSystemCalled=%s visibleTerrainDrawExecuted=%s liveRendererDrawExecuted=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s noDraw=%s liveNoDraw=%s offscreenOnly=%s validationOnly=%s validationProgramId=%d offscreenFramebufferId=%d offscreenTextureId=%d validationVertexArrayId=%d validationIndexBufferId=%d validationIndirectBufferId=%d validationParameterBufferId=%d k6FirstCommandCount=%d k6FirstCommandInstanceCount=%d k6FirstCommandFirstIndex=%d k6FirstCommandBaseVertex=%d k6FirstCommandBaseInstance=%d firstCommandCount=%d firstCommandInstanceCount=%d firstCommandFirstIndex=%d firstCommandBaseVertex=%d firstCommandBaseInstance=%d lifecycleState=%s lastLifecycleEvent=%s stale=%s requiresRebuild=%s blockerCount=%d p0BlockerCount=%d p1BlockerCount=%d p2BlockerCount=%d blockers=%s lastGlError=%s lastFailureReason=%s lastAuditOk=%s lastAuditError=%s lastAuditDurationMs=%.2f renderer=formal-isolated-mdic-draw-smoke-test formalRenderer=false liveDraw=false offscreenValidationDraw=%s",
                status.stage(),
                status.formalTerrainRendererOwnerReady(),
                status.formalViewportOwnerReady(),
                status.formalCommandGenerationOwnerReady(),
                ...
                status.realSectionDryRunReady(),
                status.isolatedMdicDrawSmokeTestReady(),
                status.offscreenValidationDrawReady(),
                ...
                status.drawCountAccepted(),
                status.geometryInputAvailable(),
                status.modelInputAvailable(),
                status.shaderInputAvailable(),
                status.isolatedGeometryBufferUsed(),
                status.originalGeometryHeapMutated(),
                status.liveGeometryHeapUsedAsMutableTarget(),
                status.formalModelIdsUsed(),
                status.placeholderModelIdsUsed(),
                status.sampleSetUsedAsFormalSource(),
                status.validationFormalModelId(),
                status.validationFormalModelIds(),
                status.validationDrawShaderUsed(),
                status.productionTerrainShaderUsed(),
                status.formalTerrainShaderReady(),
                ...
                status.bindingLayoutCompatible(),
                status.glMultiDrawElementsIndirectCountCalled(),
                status.glMultiDrawElementsIndirectCountCallScope(),
                status.mdicSectionRendererCalled(),
                status.voxyRenderSystemCalled(),
                status.visibleTerrainDrawExecuted(),
                status.liveRendererDrawExecuted(),
                ...
                status.validationVertexArrayId(),
                status.validationIndexBufferId(),
                status.validationIndirectBufferId(),
                status.validationParameterBufferId(),
    ...
    private static String formatFormalIsolatedMdicDrawAudit(ForgeFormalIsolatedMdicDrawSmokeTestAuditResult audit) {
        return String.format(
                "k7AuditOk=%s k7AuditError=%s durationMs=%.2f k1OwnerExists=%s k2ViewportOwnerExists=%s k3CommandGenerationOwnerExists=%s k4VisibilityOwnerExists=%s k5ValidationProgramExists=%s k6RealSectionDryRunExists=%s originalMdicSectionRendererInspected=%s originalShaderBindingsInspected=%s drawInputUsesK6Command=%s realSectionCommandUsed=%s syntheticDrawFixtureUsed=%s validationBuffersIsolated=%s debugMdicCommandBuffersUsedAsFormal=%s originalGeometryHeapMutated=%s offscreenFramebufferComplete=%s offscreenValidationDrawExecuted=%s offscreenReadbackOk=%s offscreenNonZeroPixelCount=%d drawCommandAccepted=%s drawCountAccepted=%s drawCommandMatchesK6=%s geometryInputAvailable=%s modelInputAvailable=%s shaderInputAvailable=%s glMultiDrawElementsIndirectCountCalled=%s visibleTerrainDrawExecuted=%s liveRendererDrawExecuted=%s minecraftMainFramebufferDrawn=%s MDICSectionRendererCalled=%s VoxyRenderSystemCalled=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s",
                audit.success(),
                audit.error(),
                audit.durationMs(),
                audit.k1OwnerExists(),
                ...
                audit.k4VisibilityOwnerExists(),
                audit.k5ValidationProgramExists(),
                audit.k6RealSectionDryRunExists(),
                audit.originalMdicSectionRendererInspected(),
                audit.originalShaderBindingsInspected(),
                audit.drawInputUsesK6Command(),
                audit.realSectionCommandUsed(),
                audit.syntheticDrawFixtureUsed(),
                audit.validationBuffersIsolated(),
                audit.debugMdicCommandBuffersUsedAsFormal(),
                audit.originalGeometryHeapMutated(),
                audit.offscreenFramebufferComplete(),
                audit.offscreenValidationDrawExecuted(),
                audit.offscreenReadbackOk(),
                audit.offscreenNonZeroPixelCount(),
                audit.drawCommandAccepted(),
                audit.drawCountAccepted(),
                audit.drawCommandMatchesK6(),
                audit.geometryInputAvailable(),
                audit.modelInputAvailable(),
                audit.shaderInputAvailable(),
                audit.glMultiDrawElementsIndirectCountCalled(),
                audit.visibleTerrainDrawExecuted(),
                audit.liveRendererDrawExecuted(),
                audit.minecraftMainFramebufferDrawn(),
                audit.mdicSectionRendererCalled(),
                audit.voxyRenderSystemCalled(),
                audit.formalDrawPipelineReady(),
                audit.formalRendererReady(),
    ...
    private static String formatFormalModelIdGeometryPathStatus(ForgeFormalModelIdSectionGeometryStats status) {
        return String.format(
                "stage=%s formalTerrainRendererOwnerReady=%s formalModelBakeryLifecycleSkeletonReady=%s formalModelStoreOwnerReady=%s realTerrainPackedRecordBridgeReady=%s realSectionDryRunReady=%s isolatedMdicDrawSmokeTestReady=%s formalModelIdGeometryPathReady=%s globalFormalModelIdGeometryPathReady=%s globalFormalModelIdGeometryEnabledForLiveRenderer=%s formalGeometrySnapshotCreated=%s formalGeometrySnapshotSectionCount=%d formalGeometrySnapshotRecordCount=%d formalGeometrySnapshotVertexCount=%d formalGeometrySnapshotByteSize=%d formalGeometryValidationBufferCreated=%s formalGeometryValidationBufferId=%d validationOnly=%s liveRendererBuffer=%s realSectionInputUsed=%s candidateSectionCount=%d acceptedSectionCount=%d rejectedSectionCount=%d acceptedRecordCount=%d rejectedRecordCount=%d candidateSectionSource=%s blockStateSourceAvailable=%s formalModelIdLookupOk=%s formalModelIdsBackedByRealBake=%s usesFormalModelIds=%s usesPlaceholderModelIds=%s sampleSetModelIdsUsed=%s sampleSetUsedAsFormalSource=%s recordsRejectedNoBlockState=%d recordsRejectedNoFormalModelId=%d recordsRejectedUnsafeModelId=%d recordsRejectedUnsupportedBlock=%d recordsRejectedPlaceholderModelId=%d recordsRejectedNoRealBake=%d formalPackedRecordsAuditOk=%s formalModelIdDecodeOk=%s formalGeometryReadbackOk=%s formalGeometryReadbackMismatchCount=%d formalGeometryModelIds=%s sampleOriginalRecord=%s sampleFormalRecord=%s sampleSectionPosition=%s originalGeometryHeapMutated=%s liveGeometryHeapUsedAsMutableTarget=%s debugGeometryHeapUsedAsFormal=%s optionalOffscreenValidationDrawExecuted=%s optionalOffscreenReadbackOk=%s visibleTerrainDrawExecuted=%s liveRendererDrawExecuted=%s formalTerrainShaderReady=%s productionTerrainShaderUsed=%s formalDrawPipelineReady=%s formalRendererReady=%s actualRendererDrawEnabled=%s MDICSectionRendererCalled=%s VoxyRenderSystemCalled=%s lifecycleState=%s lastLifecycleEvent=%s stale=%s requiresRebuild=%s blockerCount=%d p0BlockerCount=%d p1BlockerCount=%d p2BlockerCount=%d blockers=%s lastGlError=%s lastFailureReason=%s lastAuditOk=%s lastAuditError=%s lastAuditDurationMs=%.2f renderer=formal-model-id-section-geometry-path formalRenderer=false liveDraw=false",
                status.stage(),
                status.formalTerrainRendererOwnerReady(),
                status.formalModelBakeryLifecycleSkeletonReady(),
                status.formalModelStoreOwnerReady(),
                ...
                status.globalFormalModelIdGeometryEnabledForLiveRenderer(),
                status.formalGeometrySnapshotCreated(),
                status.formalGeometrySnapshotSectionCount(),
                status.formalGeometrySnapshotRecordCount(),
                status.formalGeometrySnapshotVertexCount(),
                status.formalGeometrySnapshotByteSize(),
                status.formalGeometryValidationBufferCreated(),
                status.formalGeometryValidationBufferId(),
                status.validationOnly(),
                status.liveRendererBuffer(),
                status.realSectionInputUsed(),
                status.candidateSectionCount(),
                ...
                status.recordsRejectedUnsafeModelId(),
                status.recordsRejectedUnsupportedBlock(),
                status.recordsRejectedPlaceholderModelId(),
                status.recordsRejectedNoRealBake(),
                status.formalPackedRecordsAuditOk(),
                status.formalModelIdDecodeOk(),
                status.formalGeometryReadbackOk(),
                status.formalGeometryReadbackMismatchCount(),
                status.formalGeometryModelIds(),
                status.sampleOriginalRecord(),
                status.sampleFormalRecord(),
                status.sampleSectionPosition(),
                status.originalGeometryHeapMutated(),
                status.liveGeometryHeapUsedAsMutableTarget(),
                status.debugGeometryHeapUsedAsFormal(),
                status.optionalOffscreenValidationDrawExecuted(),
                status.optionalOffscreenReadbackOk(),
                status.visibleTerrainDrawExecuted(),
                status.liveRendererDrawExecuted(),
                status.formalTerrainShaderReady(),
                status.productionTerrainShaderUsed(),
    ...
    private static String formatFormalModelIdGeometryPathAudit(ForgeFormalModelIdSectionGeometryAuditResult audit) {
        return String.format(
                "k8AuditOk=%s k8AuditError=%s durationMs=%.2f originalRenderDataFactoryInspected=%s originalModelFactoryInspected=%s originalQuadFormatInspected=%s k8InputUsesRealSectionData=%s formalModelIdsBackedByRealBake=%s placeholderIdsUsedAsFormalIds=%s sampleSetUsedAsFormalSource=%s formalGeometrySnapshotIsolated=%s originalGeometryHeapMutated=%s liveRendererConsumesK8Geometry=%s mainFramebufferDraw=%s MDICSectionRendererCalled=%s VoxyRenderSystemCalled=%s formalModelIdDecodeOk=%s formalGeometryReadbackOk=%s formalRendererReady=%s actualRendererDrawEnabled=%s",
                audit.success(),
                audit.error(),
                audit.durationMs(),
                audit.originalRenderDataFactoryInspected(),
                audit.originalModelFactoryInspected(),
                audit.originalQuadFormatInspected(),
                audit.k8InputUsesRealSectionData(),
                audit.formalModelIdsBackedByRealBake(),
                audit.placeholderIdsUsedAsFormalIds(),
                audit.sampleSetUsedAsFormalSource(),
                audit.formalGeometrySnapshotIsolated(),
                audit.originalGeometryHeapMutated(),
                audit.liveRendererConsumesK8Geometry(),
                audit.mainFramebufferDraw(),
                audit.mdicSectionRendererCalled(),
                audit.voxyRenderSystemCalled(),
                audit.formalModelIdDecodeOk(),
                audit.formalGeometryReadbackOk(),
                audit.formalRendererReady(),
                audit.actualRendererDrawEnabled()
        );
    }
    ...
    private static String formatMinimalFormalLodRendererStatus(ForgeFormalVisibleLodPreviewStats status) {
        return String.format(
                "stage=%s k23_k30_stage=%s previewPrepared=%s prepareRequested=%s prepareInProgress=%s prepareCompleted=%s "
                        + "minimalFormalLodRendererPrototypeReady=%s boundedFormalLodPatchReady=%s minimalFormalLodRendererSummary=%s "
                        + "visiblePreviewEnabled=%s visiblePreviewDrawExecuted=%s visiblePreviewFrameCount=%d minecraftMainFramebufferDrawn=%s "
                        + "visibleTerrainPreviewOnly=true debugOptInOnly=true visiblePreviewDefaultEnabled=false "
                        + "k8FormalGeometryUsed=%s k9TerrainShaderIntegrationUsed=%s k6RealSectionCommandUsed=%s syntheticDrawFixtureUsed=%s "
                        + "previewDrawCommandBucketAlignmentReady=%s previewDrawCommandDrivesDrawCount=%s productionMdicIndirectDrawUsed=%s "
                        + "formalModelIdDecodeOk=%s faceDataLookupOk=%s atlasSampleOk=%s modelDataReadOk=%s modelColourReadOk=%s "
                ...
                status.stage(),
                "K23_K30_MINIMAL_FORMAL_LOD_RENDERER_PROTOTYPE",
                status.visibleLodPreviewOwnerReady() && !status.stale(),
                status.observePrepareRequested(),
                status.observePrepareInProgress(),
                status.observePrepareCompleted(),
                minimalFormalLodRendererPrototypeReady(status),
                boundedFormalLodPatchReady(status),
                status.previewDrawCommandBucketSummary(),
                status.visiblePreviewEnabled(),
                status.visiblePreviewDrawExecuted(),
    ...
    private static boolean minimalFormalLodRendererPrototypeReady(ForgeFormalVisibleLodPreviewStats status) {
        return status.previewDrawCommandBucketSummary().contains("minimalPrototypeReady=true");
    }

    private static boolean boundedFormalLodPatchReady(ForgeFormalVisibleLodPreviewStats status) {
        return status.previewDrawCommandBucketSummary().contains("boundedPatchReady=true");
    }

    private static String formatFormalLodPreviewMovingStatus(ForgeFormalVisibleLodPreviewStats status) {
        return String.format(
                "stage=%s k31_k36_stage=K31_K36_FORMAL_LOD_PREVIEW_MOVING_PATCH_OWNER previewPrepared=%s prepareRequested=%s prepareInProgress=%s prepareCompleted=%s "
                        + "movingFormalLodPreviewReady=%s movingSummary=%s "
                        + "visiblePreviewEnabled=%s visiblePreviewDrawExecuted=%s visiblePreviewFrameCount=%d minecraftMainFramebufferDrawn=%s "
                        + "visibleTerrainPreviewOnly=true debugOptInOnly=true visiblePreviewDefaultEnabled=false "
                ...
                status.observePrepareInProgress(),
                status.observePrepareCompleted(),
                movingFormalLodPreviewReady(status),
                status.previewDrawCommandBucketSummary(),
                status.visiblePreviewEnabled(),
                status.visiblePreviewDrawExecuted(),
                status.visiblePreviewFrameCount(),
    ...
    private static boolean movingFormalLodPreviewReady(ForgeFormalVisibleLodPreviewStats status) {
        return status.previewDrawCommandBucketSummary().contains("movingOwnerReady=true")
                && status.previewDrawCommandBucketSummary().contains("movingPatchReady=true")
                && status.previewDrawCommandBucketSummary().contains("liveRendererEnabled=false");
    }

    private static String formatFormalLodPreviewExpandedStatus(ForgeFormalVisibleLodPreviewStats status) {
        return String.format(
                "stage=%s k37_k42_stage=K37_K42_EXPANDED_FORMAL_LOD_PATCH_PREVIEW previewPrepared=%s prepareRequested=%s prepareInProgress=%s prepareCompleted=%s "
                        + "expandedFormalLodPatchReady=%s expandedSummary=%s "
                        + "visiblePreviewEnabled=%s visiblePreviewDrawExecuted=%s visiblePreviewFrameCount=%d minecraftMainFramebufferDrawn=%s "
                        + "visibleTerrainPreviewOnly=true debugOptInOnly=true visiblePreviewDefaultEnabled=false "
                        + "k8FormalGeometryUsed=%s syntheticDrawFixtureUsed=%s formalModelIdDecodeOk=%s faceDataLookupOk=%s atlasSampleOk=%s "
                        + "originalGeometryHeapMutated=%s debugMdicCommandBuffersUsedAsFormal=%s sampleSetUsedAsFormalSource=%s "
                ...
                status.observePrepareInProgress(),
                status.observePrepareCompleted(),
                expandedFormalLodPatchReady(status),
                status.previewDrawCommandBucketSummary(),
                status.visiblePreviewEnabled(),
                status.visiblePreviewDrawExecuted(),
                status.visiblePreviewFrameCount(),
    ...
    private static boolean expandedFormalLodPatchReady(ForgeFormalVisibleLodPreviewStats status) {
    ...
    private static String formatFormalVisibleLodPreviewStatus(ForgeFormalVisibleLodPreviewStats status) {
        return String.format(
                ...
                status.visibleLodPreviewOwnerReady(),
                status.visiblePreviewEnabled(),
                status.visiblePreviewDefaultEnabled(),
                status.visiblePreviewDefaultDisabledVerified(),
                status.debugOptInOnly(),
                status.visiblePreviewWasEnabledDuringQa(),
                status.visiblePreviewDisabledAfterQa(),
                status.visiblePreviewDrawExecuted(),
                status.visiblePreviewFrameCount(),
                status.visibleTerrainPreviewOnly(),
                status.minecraftMainFramebufferDrawn(),
                status.productionLiveRendererDrawExecuted(),
                status.renderHookRegistered(),
                status.renderHookName(),
                status.renderHookScope(),
                status.drawInputSource(),
                status.k8FormalGeometryUsed(),
                status.k9TerrainShaderIntegrationUsed(),
                status.k6RealSectionCommandUsed(),
                status.syntheticDrawFixtureUsed(),
                status.worldSpacePreview(),
                status.previewSectionWorldPosition(),
                status.previewCameraRelativeTransformOk(),
                status.projectionMatrixUsed(),
                status.modelViewMatrixUsed(),
                status.k9TerrainShaderAdapterReused(),
                status.visiblePreviewShaderProgramCompileAttempted(),
                status.visiblePreviewShaderProgramCompileOk(),
                status.visiblePreviewShaderProgramLinkOk(),
                status.visiblePreviewShaderProgramId(),
                status.visiblePreviewDrawCallOk(),
                status.visiblePreviewLightweightDrawPath(),
                status.visiblePreviewDirectDrawUsed(),
                status.visiblePreviewDrawArraysUsed(),
                status.visiblePreviewIndirectCountDrawUsed(),
                status.visiblePreviewValidationChecksOnly(),
                status.visiblePreviewCommandIndexCount(),
                status.visiblePreviewDrawIndexCount(),
                status.visiblePreviewDrawIndexCap(),
                status.visiblePreviewDrawCountCapped(),
                status.visiblePreviewLastGlError(),
                status.formalModelIdDecodeOk(),
                status.faceDataLookupOk(),
                status.atlasSampleOk(),
                status.modelDataReadOk(),
                status.modelColourReadOk(),
                status.visiblePreviewReadbackOk(),
                status.visiblePreviewNonZeroPixelCount(),
                status.visiblePreviewChecksum(),
                status.validationFormalModelId(),
                status.validationFace(),
                status.validationFormalModelIds(),
                status.geometryBufferId(),
                ...
                status.observePrepareInProgress(),
                status.observePrepareCompleted(),
                status.observePrepareFailedSafely(),
                status.observeAutoEnabledAfterPrepare(),
                status.observePrepareFrameBudgetExceeded(),
                status.observePrepareStep(),
                status.observePrepareFrameCount(),
                status.observePrepareNextAttemptFrame(),
                status.lastObservePrepareStepName(),
                status.lastObservePrepareFailureReason(),
                status.k12K13Stage(),
                status.prepareRunCount(),
                status.prepareReuseCount(),
                status.prepareTimingReady(),
                status.prepareReuseReady(),
                status.lastPrepareReusedExistingResources(),
                status.unnecessaryRebuildDetected(),
                status.prepareStepBudgetExceeded(),
                status.prepareStepBudgetWarnMs(),
                status.lastPrepareTotalDurationMs(),
                status.lastPrepareStepDurationMs(),
                status.maxPrepareStepDurationMs(),
                status.slowestPrepareStepName(),
                status.prepareBlockingStep(),
                status.prepareBlockingReason(),
                status.prepareEnsureWorldDurationMs(),
                status.prepareK6DurationMs(),
                status.prepareK7DurationMs(),
                status.prepareK8DurationMs(),
                status.prepareK9DurationMs(),
                status.prepareK10DurationMs(),
                status.prepareTimingSummary(),
                status.visibleLodPreviewOwnerReady() && !status.stale(),
                status.observeEnableFailedSafely() && status.lastObserveEnableFailureReason().contains("preview-not-prepared"),
    ...
    private static String formatFormalVisibleLodPreviewAudit(ForgeFormalVisibleLodPreviewAuditResult audit) {
        return String.format(
                "k10AuditOk=%s k10AuditError=%s durationMs=%.2f prerequisitesChecked=%s previewDisabledByDefault=%s explicitOptInRequired=%s drawInputUsesK8AndK9=%s placeholderIdsUsedAsFormalIds=%s sampleSetUsedAsFormalSource=%s originalGeometryHeapMutated=%s debugCommandBuffersUsedAsFormal=%s visiblePreviewDrawExecuted=%s minecraftMainFramebufferDrawn=%s previewDisabledAfterQa=%s MDICSectionRendererCalled=%s VoxyRenderSystemCalled=%s formalRendererReady=%s actualRendererDrawEnabled=%s",
                audit.success(),
                audit.error(),
                audit.durationMs(),
                audit.prerequisitesChecked(),
                audit.previewDisabledByDefault(),
                audit.explicitOptInRequired(),
                audit.drawInputUsesK8AndK9(),
                audit.placeholderIdsUsedAsFormalIds(),
                audit.sampleSetUsedAsFormalSource(),
                audit.originalGeometryHeapMutated(),
                audit.debugCommandBuffersUsedAsFormal(),
                audit.visiblePreviewDrawExecuted(),
                audit.minecraftMainFramebufferDrawn(),
                audit.previewDisabledAfterQa(),
                audit.mdicSectionRendererCalled(),
                audit.voxyRenderSystemCalled(),
                audit.formalRendererReady(),
                audit.actualRendererDrawEnabled()
    ...
    static String formatFormalRendererStatus(ForgeFormalRendererStats status) {
        return String.format(
                ...
                status.formalIsolatedMdicDrawSmokeTestReady(),
                status.offscreenValidationDrawReady(),
                status.offscreenValidationDrawExecuted(),
                status.offscreenValidationReadbackOk(),
                status.offscreenValidationOnly(),
                status.offscreenOnly(),
                status.realSectionCommandUsedForOffscreenDraw(),
                status.syntheticDrawFixtureUsed(),
                status.formalDrawPipelineReady(),
                status.globalFormalModelIdGeometryReady(),
                status.formalModelIdGeometryPathReady(),
                ...
                status.formalVisibleLodPreviewReady(),
                status.visiblePreviewDefaultDisabled(),
                status.visiblePreviewEnabled(),
                status.visiblePreviewDrawExecuted(),
                status.minecraftMainFramebufferDrawn(),
                status.visibleTerrainPreviewOnly(),
                status.productionLiveRendererDrawExecuted(),
                status.visiblePreviewUsesFormalInputs(),
                status.formalTerrainShaderReady(),
                status.previewSystemsSeparated(),
                status.sampleSetUsedAsFormalSource(),
                status.terrainDrawStarted(),
    ...
    static int applyPresetFormalCommandGenerationOwnerNoDraw(CommandSourceStack source) {
        ...
        ForgeFormalMdicViewportStats viewportStatus = ForgeVoxyInstance.INSTANCE.getFormalMdicViewportOwner().enable("preset-formal-command-generation-owner-no-draw");
        ForgeFormalCommandGenerationStats enableStatus = ForgeVoxyInstance.INSTANCE.getFormalCommandGenerationOwner().enable("preset-formal-command-generation-owner-no-draw");
        ForgeFormalCommandGenerationStats checkStatus = ForgeVoxyInstance.INSTANCE.getFormalCommandGenerationOwner().check("preset-formal-command-generation-owner-no-draw");
        ...
        ForgeFormalTerrainRendererStats terrainOwnerStatus = ForgeVoxyInstance.INSTANCE.getFormalTerrainRendererOwner().check("preset-formal-command-generation-owner-no-draw");
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("preset-formal-command-generation-owner-no-draw");
        String message = "Voxy preset formal_command_generation_owner_no_draw: runtime-only K3 formal command generation ownership skeleton applied, not written to toml. "
                + "The preset records the DrawCommand layout, draw-count layout, cmdgen input/output contract, and formal producer ownership, but does not run cmdgen.comp, glDispatchComputeIndirect, glMultiDrawElementsIndirectCountARB, MDICSectionRenderer, VoxyRenderSystem, or debug renderers. "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K3 owner remains no-draw and can still report static readiness. ")
    ...
    static int applyPresetFormalVisibilityOwnerNoDraw(CommandSourceStack source) {
        ...
        ForgeFormalMdicViewportStats viewportStatus = ForgeVoxyInstance.INSTANCE.getFormalMdicViewportOwner().enable("preset-formal-visibility-owner-no-draw");
        ForgeFormalCommandGenerationStats commandStatus = ForgeVoxyInstance.INSTANCE.getFormalCommandGenerationOwner().enable("preset-formal-visibility-owner-no-draw");
        ForgeFormalVisibilityStats enableStatus = ForgeVoxyInstance.INSTANCE.getFormalVisibilityOwner().enable("preset-formal-visibility-owner-no-draw");
        ForgeFormalVisibilityStats checkStatus = ForgeVoxyInstance.INSTANCE.getFormalVisibilityOwner().check("preset-formal-visibility-owner-no-draw");
        ...
        ForgeFormalTerrainRendererStats terrainOwnerStatus = ForgeVoxyInstance.INSTANCE.getFormalTerrainRendererOwner().check("preset-formal-visibility-owner-no-draw");
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("preset-formal-visibility-owner-no-draw");
        String message = "Voxy preset formal_visibility_owner_no_draw: runtime-only K4 formal visibility/render-list ownership skeleton applied, not written to toml. "
                + "The preset records the formal visibility input contract, render-list/indirect-lookup output contract, and a provisional CPU candidate snapshot, but does not implement traversal, run cmdgen.comp, call glMultiDrawElementsIndirectCountARB, MDICSectionRenderer, VoxyRenderSystem, or promote debug planners. "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K4 owner remains no-draw and can still report static readiness. ")
    ...
    static int applyPresetFormalCmdgenGpuValidationNoDraw(CommandSourceStack source) {
        ...
        ForgeFormalCmdgenGpuValidationStats status = ForgeVoxyInstance.INSTANCE.getFormalCmdgenGpuValidator().build();
        ForgeFormalCmdgenGpuValidationAuditResult audit = ForgeVoxyInstance.INSTANCE.getFormalCmdgenGpuValidator().createAuditStatusSnapshot();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("preset-formal-cmdgen-gpu-validation-no-draw");
        String message = "Voxy preset formal_cmdgen_gpu_validation_no_draw: runtime-only K5 formal command-generation GPU validation applied, not written to toml. "
                + "The preset creates tiny isolated validation-only buffers, compiles and dispatches an audit-only compute shader, reads back DrawCommand/draw-count/position-scratch outputs, and does not submit those commands to a draw call. "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K5 uses a synthetic validation fixture and remains no-draw. ")
                + formatFormalCmdgenGpuValidationStatus(status)
                + " "
                + formatFormalCmdgenGpuValidationAudit(audit)
                + " "
    ...
    static int applyPresetFormalIsolatedMdicDrawOffscreen(CommandSourceStack source) {
        ...
        ForgeFormalIsolatedMdicDrawSmokeTestStats status = ForgeVoxyInstance.INSTANCE.getFormalIsolatedMdicDrawSmokeTest().build();
        ForgeFormalIsolatedMdicDrawSmokeTestAuditResult audit = ForgeVoxyInstance.INSTANCE.getFormalIsolatedMdicDrawSmokeTest().createAuditStatusSnapshot();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("preset-formal-isolated-mdic-draw-offscreen");
        String message = "Voxy preset formal_isolated_mdic_draw_offscreen: runtime-only K7 formal isolated MDIC draw smoke test applied, not written to toml. "
                + "The preset consumes K6 real-section command data, builds K7-owned validation buffers and an offscreen framebuffer, executes only an offscreen validation draw, and does not draw into the Minecraft main framebuffer or enable live terrain rendering. "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K7 success requires a real-section K6 command and should be treated as partial if ready=false. ")
                + formatFormalIsolatedMdicDrawStatus(status)
                + " "
                + formatFormalIsolatedMdicDrawAudit(audit)
    ...
    static int applyPresetFormalModelIdGeometryPathNoLiveDraw(CommandSourceStack source) {
        ...
        ForgeFormalModelIdSectionGeometryStats status = ForgeVoxyInstance.INSTANCE.getFormalModelIdSectionGeometryPath().build();
        ForgeFormalModelIdSectionGeometryAuditResult audit = ForgeVoxyInstance.INSTANCE.getFormalModelIdSectionGeometryPath().createAuditStatusSnapshot();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("preset-formal-model-id-geometry-path-no-live-draw");
        String message = "Voxy preset formal_model_id_geometry_path_no_live_draw: runtime-only K8 formal model-id section geometry path applied, not written to toml. "
                + "The preset builds a K8-owned isolated formal geometry snapshot/validation buffer from real section records with formal model ids; it does not mutate the original GL geometry heap, submit live draw, call MDICSectionRenderer, or call VoxyRenderSystem. "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K8 success requires real section input and should be treated as partial if ready=false. ")
                + formatFormalModelIdGeometryPathStatus(status)
                + " "
                + formatFormalModelIdGeometryPathAudit(audit)
    ...
    static int applyPresetFormalVisibleLodPreviewDebug(CommandSourceStack source) {
        ...
        ForgeFormalVisibleLodPreviewStats status = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview().runQa();
        ForgeFormalVisibleLodPreviewAuditResult audit = ForgeVoxyInstance.INSTANCE.getFormalVisibleLodPreview().createAuditStatusSnapshot();
        ForgeFormalRendererStats rendererStatus = ForgeVoxyInstance.INSTANCE.getFormalRendererManager().checkReadiness("preset-formal-visible-lod-preview-debug");
        String message = "Voxy preset formal_visible_lod_preview_debug: runtime-only K10 visible formal LoD preview applied, not written to toml. "
                + "The preset explicitly enables the K10 preview for a short QA window, then the render hook auto-disables it; production live renderer readiness remains false. "
                + (engineReady ? "WorldEngine is active. " : "No active client world was found; K10 success requires real section input and should be treated as partial if ready=false. ")
                + formatFormalVisibleLodPreviewStatus(status)
                + " "
                + formatFormalVisibleLodPreviewAudit(audit)
