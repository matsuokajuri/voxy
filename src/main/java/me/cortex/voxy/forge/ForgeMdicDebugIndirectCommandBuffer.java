...
final class ForgeMdicDebugIndirectCommandBuffer {
    static final int WORDS_PER_COMMAND = 4;
    ...
    boolean upload(ForgeMdicCommandList commandList, int maxCommands, int maxRecords) {
        requireRenderThread("upload MDIC debug indirect command buffer");
        if (commandList == null || !commandList.isValid()) {
            this.closeOnRenderThread();
            this.lastUploadError = "command-list-empty";
            return false;
        }
        IndirectBudget budget = IndirectBudget.from(commandList, maxCommands, maxRecords);
        ...
        try {
            ...
            for (ForgeMdicCommand command : commandList.commands()) {
                ...
                MemoryUtil.memPutInt(ptr + offset, vertexCount);
                MemoryUtil.memPutInt(ptr + offset + 4L, 1);
                MemoryUtil.memPutInt(ptr + offset + 8L, 0);
                MemoryUtil.memPutInt(ptr + offset + 12L, commandIndex);
    ...
    int[] readbackWords() {
        requireRenderThread("read back MDIC debug indirect command buffer");
        if (this.bufferId == 0 || this.commandCount <= 0 || this.bytes <= 0L) {
            throw new IllegalStateException("MDIC debug indirect command buffer is not created");
        }
        if (this.bytes > Integer.MAX_VALUE) {
            throw new IllegalStateException("MDIC debug indirect command buffer is too large to audit: " + this.bytes);
        }
    ...
    private record IndirectBudget(int commandCount, long bytes) {
        static IndirectBudget from(ForgeMdicCommandList commandList, int maxCommands, int maxRecords) {
            ...
            return new IndirectBudget(count, (long) count * BYTES_PER_COMMAND);
        }
    }
}
