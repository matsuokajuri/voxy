...
record ForgeGpuGeometryDecodedMetadata(long position, int aabb, int geometryPtr, int[] offsets, int[] deltas, int itemCount) {
    static final int BUCKET_COUNT = 8;

    static ForgeGpuGeometryDecodedMetadata decode(int[] words) {
        ...
        return new ForgeGpuGeometryDecodedMetadata(position, words[2], geometryPtr, offsets, deltas, itemCount);
    }
