...
final class ForgeModelSampleSetBuffer {
    ...
    boolean upload(List<ForgeRealModelStoreSampleRecord> records, long generation, String dimensionId) {
        requireRenderThread("upload model sample set buffers");
        if (records == null || records.isEmpty()) {
            this.closeOnRenderThread();
            this.lastUploadError = "no-model-sample-set-records";
            return false;
        }

        long dataBytes = (long) records.size() * ForgeRealModelStoreSampleRecord.BYTES;
        ...
        try {
            ...
            for (int i = 0; i < records.size(); i++) {
                MemoryUtil.memPutInt(colourPtr + i * (long) Integer.BYTES, records.get(i).modelColour());
            }
    ...
    int[] readbackModelDataWords() {
        requireRenderThread("read back model sample set modelData buffer");
        if (!this.isModelDataCreated()) {
            throw new IllegalStateException("model sample set modelData buffer is not created");
        }
        int wordsCount = this.recordCount * ForgeRealModelStoreSampleRecord.WORDS;
    ...
    int[] readbackModelColours() {
        requireRenderThread("read back model sample set modelColour buffer");
        if (!this.isModelColourCreated()) {
            throw new IllegalStateException("model sample set modelColour buffer is not created");
        }
