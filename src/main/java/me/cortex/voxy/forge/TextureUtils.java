...
final class TextureUtils {
    ...
    static int getWrittenPixelCount(ColourDepthTextureData texture, int checkMode) {
        int count = 0;
    ...
    static boolean hasTranslucentPixel(ColourDepthTextureData texture) {
        for (int i = 0; i < texture.colour().length; i++) {
    ...
    static boolean isSolidWhereDrawn(ColourDepthTextureData texture) {
        for (int i = 0; i < texture.colour().length; i++) {
            int alpha = texture.colour()[i] >>> 24;
            int depth = texture.depth()[i];
    ...
    static int computeFaceTint(ColourDepthTextureData texture, int checkMode) {
    ...
    static float computeDepth(ColourDepthTextureData texture, int mode, int checkMode) {
        int[] colourData = texture.colour();
        int[] depthData = texture.depth();
        long a = 0;
        long b = 0;
    ...
    static long[] generateMask(ColourDepthTextureData texture, int checkMode) {
        return generateMask(texture, checkMode, new long[texture.width() * texture.height() / 64]);
    }

    static long[] generateMask(ColourDepthTextureData texture, int checkMode, long[] outMsk) {
    ...
    static int[] computeBounds(ColourDepthTextureData texture, int checkMode) {
        int minX = 0;
        minXCheck:
        do {
            for (int y = 0; y < texture.height(); y++) {
