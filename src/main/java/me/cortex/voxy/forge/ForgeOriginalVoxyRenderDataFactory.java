...
final class ForgeOriginalVoxyRenderDataFactory {
    ...
    private final ForgeOriginalVoxyModelFactory modelMan;
    ...
    private final ForgeOriginalVoxyOccupancySet occupancy;

    //Wont work for double sided quads
    private final class Mesher extends ForgeOriginalVoxyScanMesher2D {
        ...
        protected void emitQuad(int x, int z, int length, int width, long data) {
            ...
            ForgeOriginalVoxyRenderDataFactory.this.quadCount++;
            ...
            long bufferOffset = (ForgeOriginalVoxyRenderDataFactory.this.quadCounters[bufferIdx]++)*8L + bufferIdx*8L*(1<<16);
            MemoryUtil.memPutLong(ForgeOriginalVoxyRenderDataFactory.this.quadBufferPtr + bufferOffset, quad);


            //Update AABB bounds
            if (axis == 0) {//Y
                ForgeOriginalVoxyRenderDataFactory.this.minY = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minY, auxPos);
                ForgeOriginalVoxyRenderDataFactory.this.maxY = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxY, auxPos);

                ForgeOriginalVoxyRenderDataFactory.this.minX = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minX, x);
                ForgeOriginalVoxyRenderDataFactory.this.maxX = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxX, x + length);

                ForgeOriginalVoxyRenderDataFactory.this.minZ = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minZ, z);
                ForgeOriginalVoxyRenderDataFactory.this.maxZ = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxZ, z + width);
            } else if (axis == 1) {//Z
                ForgeOriginalVoxyRenderDataFactory.this.minZ = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minZ, auxPos);
                ForgeOriginalVoxyRenderDataFactory.this.maxZ = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxZ, auxPos);

                ForgeOriginalVoxyRenderDataFactory.this.minX = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minX, x);
                ForgeOriginalVoxyRenderDataFactory.this.maxX = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxX, x + length);

                ForgeOriginalVoxyRenderDataFactory.this.minY = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minY, z);
                ForgeOriginalVoxyRenderDataFactory.this.maxY = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxY, z + width);
            } else {//X
                ForgeOriginalVoxyRenderDataFactory.this.minX = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minX, auxPos);
                ForgeOriginalVoxyRenderDataFactory.this.maxX = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxX, auxPos);

                ForgeOriginalVoxyRenderDataFactory.this.minY = Math.min(ForgeOriginalVoxyRenderDataFactory.this.minY, x);
                ForgeOriginalVoxyRenderDataFactory.this.maxY = Math.max(ForgeOriginalVoxyRenderDataFactory.this.maxY, x + length);
    ...
    public ForgeOriginalVoxyRenderDataFactory(WorldEngine world, ForgeOriginalVoxyModelFactory modelManager, boolean emitMeshlets) {
        this(world, modelManager, emitMeshlets, false);
    }
    public ForgeOriginalVoxyRenderDataFactory(WorldEngine world, ForgeOriginalVoxyModelFactory modelManager, boolean emitMeshlets, boolean generateOccupancy) {
        this.world = world;
        this.modelMan = modelManager;
        if (generateOccupancy && BUILD_OCCUPANCY_SET) {
            this.occupancy = new ForgeOriginalVoxyOccupancySet();
        } else {
            this.occupancy = null;
        }
    }
    ...
    private static long getQuadTyping(long metadata) {//2 bits
        return 0b111L&(0b000_000_010_100L>>(ForgeOriginalVoxyModelQueries._isTranslucent(metadata)*6+ForgeOriginalVoxyModelQueries._isDoubleSided(metadata)*3));
    }
    ...
    private static long packPartialQuadData(int modelId, long state, long metadata) {
        ...
        lightAndBiome &= ~(ForgeOriginalVoxyModelQueries._notIsBiomeColoured(metadata) * (0x1FFL << 46));//46 not 47 because is already shifted by 1 THIS WASTED 4 HOURS ;-; aaaaaAAAAAA
        lightAndBiome &= ~(ForgeOriginalVoxyModelQueries._isFullyOpaque(metadata)*(0xFFL << 55));//If its fully opaque it always uses neighbor light?
            ...
            for (int j = 0; j < 64; i++, j++) {
                ...
                if (Mapper.isAir(block)) {//If it is air, just emit lighting
                    ...
                    if (modelId == 0) {//modelId == 0, its basicly air so set it as air
                        ...
                        notEmpty |= 1L << j;
                        opaque |= ForgeOriginalVoxyModelQueries._isFullyOpaque(modelMetadata)<<j;
                        pureFluid |= ForgeOriginalVoxyModelQueries._isFluid(modelMetadata)<<j;
                        partialFluid |= ForgeOriginalVoxyModelQueries._containsFluid(modelMetadata)<<j;
                    }
                }
            }
            if (notEmpty != 0) {
    ...
    private static boolean shouldMeshNonOpaqueBlockFace(int face, long quad, long meta, long neighborQuad, long neighborMeta) {
        if (((quad^neighborQuad)&(0xFFFFL<<26))==0 && (DISABLE_CULL_SAME_OCCLUDES || ForgeOriginalVoxyModelQueries.cullsSame(meta))) return false;//This is a hack, if the neigbor and this are the same, dont mesh the face// TODO: FIXME
        if (!ForgeOriginalVoxyModelQueries.faceExists(meta, face)) return false;//Dont mesh if no face
        if (ForgeOriginalVoxyModelQueries.faceCanBeOccluded(meta, face)) //TODO: maybe enable this
          if (ForgeOriginalVoxyModelQueries.faceOccludes(neighborMeta, face^1)) return false;
        return true;
    }
    ...
    private static void meshNonOpaqueFace(int face, long quad, long meta, long neighborQuad, long neighborMeta, Mesher mesher) {
        if (shouldMeshNonOpaqueBlockFace(face, quad, meta, neighborQuad, neighborMeta)) {
            mesher.putNext(applyQuadLight(
                    (long) (face&1) |
                    (quad&~LM) |
                    ((ForgeOriginalVoxyModelQueries.faceUsesSelfLighting(meta, face)?quad:neighborQuad) & LM),
                    meta));
        } else {
            mesher.skip(1);
        }
    ...
    private static long applyQuadLight(long quad, long selfmeta) {
        ...
        long bl = quad&BLMSK;
        bl = Math.max(bl, ForgeOriginalVoxyModelQueries.lightEmission(selfmeta)<<(55+4));
        quad &= ~(BLMSK);
        quad |= bl;
        return quad;
    }
                ...
                while (msk != 0) {
...

                    {
                        ...
                        if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
                            ...
                            boolean culls = false;
                            culls |= ((selfModel^nextModel)&(0xFFFFL<<26))==0&&ForgeOriginalVoxyModelQueries.cullsSame(neighbor);
                            culls |= ForgeOriginalVoxyModelQueries.faceOccludes(neighbor, (axis << 1) | (1 - facingForward));
                            if (culls) {
                ...
                while (msk != 0) {
...

                    {
                        ...
                        if (nib != 0) {//Not air
                            ...
                            if (ForgeOriginalVoxyModelQueries.isFullyOpaque(meta)) {//Dont mesh this face
                                this.blockMesher.skip(1);
                                continue;
                            }
                            ...
                            if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
                                boolean culls = false;
                                culls |= cid==((A>>26)&0xFFFF)&&ForgeOriginalVoxyModelQueries.cullsSame(meta);
                                culls |= ForgeOriginalVoxyModelQueries.faceOccludes(meta, (axis << 1) | (1 - side));
                                if (culls) {
                                    this.blockMesher.skip(1);
                                    continue;
                                }
                ...
                while (msk != 0) {
...

                    {
                        ...
                        if (CHECK_NEIGHBOR_FACE_OCCLUSION) {//TODO:SELF OCCLUSION
                            if (ForgeOriginalVoxyModelQueries.faceOccludes(this.sectionData[bi + 1], (axis << 1) | (1 - facingForward))) {
                                this.blockMesher.skip(1);
                                continue;
                            }
                        }
                        ...
                        if (ForgeOriginalVoxyModelQueries.containsFluid(Am)) {
                        ...
                        //if (!ForgeOriginalVoxyModelQueries.faceUsesSelfLighting(Am, facingForward|(axis*2))) {//TODO: check this is right
                ...
                while (msk != 0) {
...

                    {
                        ...
                        if (Mapper.getBlockId(neighborId) != 0) {//Not air
                            ...
                            if (ForgeOriginalVoxyModelQueries.containsFluid(meta)) {
                                modelId = this.modelMan.getFluidClientStateId(modelId);
                            }
                            if (ForgeOriginalVoxyModelQueries.cullsSame(Am)) {
                            ...
                            if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
                                if (ForgeOriginalVoxyModelQueries.faceOccludes(meta, (axis << 1) | (1 - side))) {
                                    this.blockMesher.skip(1);
                                    continue;
                                }
                            }
                ...
                while (msk != 0) {
...

                    {
                        ...
                        if (Mapper.getBlockId(neighborId) != 0) {//Not air
                            ...
                            if (ForgeOriginalVoxyModelQueries.cullsSame(Am) && modelId == ((A>>26)&0xFFFF)) {//TODO: FIXME, this technically isnt correct as need to check self occulsion, thinks?
                        ...
                        if (ForgeOriginalVoxyModelQueries.cullsSame(nB) && (nA&(0xFFFFL<<26)) == (A&(0xFFFFL<<26))) {//TODO: FIXME, this technically isnt correct as need to check self occulsion, thinks?
                            //TODO: check self occlsuion in the if statment
                            failB = true;
                        } else {
                            if (ForgeOriginalVoxyModelQueries.faceOccludes(nB, (axis << 1) | (side))) {
                                failB = true;
                            }
                        }
                        ...
                        //TODO: LIGHTING
                        if (ForgeOriginalVoxyModelQueries.faceExists(Am, (axis<<1)|1) && ((side==1&&!fail) || (side==0&&!failB))) {
                            this.blockMesher.putNext(applyQuadLight(
                                    (long) (false ? 0L : 1L) |
                                    A |
                                    0,//((ForgeOriginalVoxyModelQueries.faceUsesSelfLighting(B, (axis<<1)|1)?A:) & (0xFFL << 55))//TODO:THIS
                                    Am)
                            );
                        } else {
                            this.blockMesher.skip(1);
                        ...
                        if (ForgeOriginalVoxyModelQueries.faceExists(Am, (axis<<1)|0) && ((side==0&&!fail) || (side==1&&!failB))) {
        ...
        for (int y = 0; y < 32; y++) {
            ...
            for (int z = 0; z < 32; z++) {
                ...
                if (z == 30 && partialHasCount != 0) {//Hackfix for incremental count overflow issue
                    ...
                    //Clear the sum
                    sumA &= ~(expandBitsLong(Integer.toUnsignedLong(partialHasCount), X_I_MSK)*0x1F);
                    sumB &= ~(expandBitsLong(Integer.toUnsignedLong(partialHasCount)>>11, X_I_MSK)*0x1F);
                    sumC &= ~(expandBitsLong(Integer.toUnsignedLong(partialHasCount)>>22, X_I_MSK)*0x1F);
                }
                ...
                {//Dont need this as can just increment everything then -1 in mask
                    ...
                    sumA += expandBitsLong(imsk, X_I_MSK);
                    sumB += expandBitsLong(imsk>>11, X_I_MSK);
                    sumC += expandBitsLong(imsk>>22, X_I_MSK);
                }*/
                ...
                while (iter!=0) {
                    ...
                    int facingForward = ((faceForwardMsk>>index)&1);
                    {
                        ...
                        if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
                            ...
                            if (ForgeOriginalVoxyModelQueries.faceOccludes(this.sectionData[iB + 1], (2 << 1) | (1 - facingForward))) {
                                mesher.skip(1);
                                continue;
                            }
                        }
            ...
            for (int z = 0; z < 32; z++) {
                ...
                if ((msk & 1) != 0) {//-x
                    ...
                    if (Mapper.getBlockId(neighborId) != 0) {//Not air
                        ...
                        if (ForgeOriginalVoxyModelQueries.isFullyOpaque(meta)) {
                            oki = false;
                        } else if (CHECK_NEIGHBOR_FACE_OCCLUSION && ForgeOriginalVoxyModelQueries.faceOccludes(meta, (2 << 1) | (1 - 0))) {
                ...
                if ((msk & (1<<31)) != 0) {//+x
                    ...
                    if (Mapper.getBlockId(neighborId) != 0) {//Not air
                        ...
                        if (ForgeOriginalVoxyModelQueries.isFullyOpaque(meta)) {
                            oki = false;
                        } else if (CHECK_NEIGHBOR_FACE_OCCLUSION && ForgeOriginalVoxyModelQueries.faceOccludes(meta, (2 << 1) | (1 - 1))) {
                            //TODO check self occlsion
                            oki = false;
                        }
                    }
            ...
            for (int z = 0; z < 32; z++) {
                ...
                while (iter!=0) {
                    ...
                    int facingForward = ((faceForwardMsk>>index)&1);
                    {
                        ...
                        if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
                            if (ForgeOriginalVoxyModelQueries.faceOccludes(this.sectionData[bi + 1], (2 << 1) | (1 - facingForward))) {
                                //TODO check self occlsion
                                mesher.skip(1);
                                continue;
                            }
            ...
            for (int z = 0; z < 32; z++) {
                ...
                if ((msk & 1) != 0) {//-x
                    ...
                    if (Mapper.getBlockId(neighborId) != 0) {//Not air
                        ...
                        //Check neighbor face
                        if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
                            if (ForgeOriginalVoxyModelQueries.faceOccludes(meta, (2 << 1) | (1-0))) {
                                oki = false;
                            }
                        }
                ...
                if ((msk & (1<<31)) != 0) {//+x
                    ...
                    if (Mapper.getBlockId(neighborId) != 0) {//Not air
                        ...
                        //Check neighbor face
                        if (CHECK_NEIGHBOR_FACE_OCCLUSION) {
                            if (ForgeOriginalVoxyModelQueries.faceOccludes(meta, (2 << 1) | (1-1))) {
                                oki = false;
                            }
                        }
    ...
    private static void dualMeshNonOpaqueOuterX(int side, long quad, long meta, int neighborAId, int neighborLight, long neighborAMeta, long neighborBQuad, long neighborBMeta, Mesher ma, Mesher mb) {
        ...
        if ((neighborAId==0 && ForgeOriginalVoxyModelQueries.faceExists(meta, ((2<<1)|0)^side))||(neighborAId!=0&&shouldMeshNonOpaqueBlockFace(((2<<1)|0)^side, quad, meta, ((long)neighborAId)<<26, neighborAMeta))) {
            ma.putNext(applyQuadLight(
                    ((long)side)|
                    (quad&~LM) |
                    (ForgeOriginalVoxyModelQueries.faceUsesSelfLighting(meta, ((2<<1)|0)^side)?quad:(((long)neighborLight)<<55)),
                    meta
                    )
            );
        } else {
        ...
        if (shouldMeshNonOpaqueBlockFace(((2<<1)|1)^side, quad, meta, neighborBQuad, neighborBMeta)) {
            mb.putNext(applyQuadLight(
                    ((long)(side^1))|
                    (quad&~LM) |
                    ((ForgeOriginalVoxyModelQueries.faceUsesSelfLighting(meta, ((2<<1)|1)^side)?quad:neighborBQuad)&(0xFFL<<55)),
                    meta
                    )
            );
        } else {
    ...
    public ForgeOriginalVoxyBuiltSection generateMesh(WorldSection section) {
        ...
        if ((neighborMskAndFlags&(1<<31))!=0) {//We failed to get everything so throw exception
            throw new ForgeOriginalVoxyIdNotYetComputedException(neighborMskAndFlags&((1<<20)-1), true);
        }
        ...
        if (this.quadCount == 0) {
            return ForgeOriginalVoxyBuiltSection.emptyWithChildren(section.key, section.getNonEmptyChildren());
        }
    ...
    private static long expandBitsLong(long value, long mask) {
        long result = 0L;
        int inputBit = 0;
        for (int bit = 0; bit < Long.SIZE; bit++) {
            long bitMask = 1L << bit;
            if ((mask & bitMask) != 0L) {
                if ((value & (1L << inputBit)) != 0L) {
                    result |= bitMask;
                }
                inputBit++;
            }
