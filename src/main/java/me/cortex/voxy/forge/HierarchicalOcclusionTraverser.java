...
final class HierarchicalOcclusionTraverser extends TrackedObject {
    ...
    private final GlBuffer requestBuffer = new GlBuffer(MAX_REQUEST_QUEUE_SIZE * 8L + 8L).zero();
    private final GlBuffer nodeBuffer;
    private final GlBuffer uniformBuffer = new GlBuffer(1024).zero();
    private final GlBuffer topNodeIds = new GlBuffer(MAX_QUEUE_SIZE * 4L).zero();
    private final GlBuffer queueMetaBuffer = new GlBuffer(4L * 4L * MAX_ITERATIONS).zero();
    private final GlBuffer scratchQueueA = new GlBuffer(MAX_QUEUE_SIZE * 4L).zero();
    private final GlBuffer scratchQueueB = new GlBuffer(MAX_QUEUE_SIZE * 4L).zero();
    ...
    private void traverseInternal() {
        ...
        long ptr = UploadStream.instance().upload(this.queueMetaBuffer.id, 0L, 16L * MAX_ITERATIONS);
