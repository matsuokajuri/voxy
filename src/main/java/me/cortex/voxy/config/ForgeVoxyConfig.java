...
public final class ForgeVoxyConfig {
    ...
    public static final ForgeConfigSpec.IntValue MDIC_DEBUG_DRAW_MAX_DRAW_COUNT;
    ...
    public static final ForgeConfigSpec.DoubleValue MDIC_DEBUG_DRAW_FRAME_BUDGET_MS;
    ...
    static {
        ...
        MDIC_COMMAND_BUCKET_AWARE = builder
                .comment("Plans one MDIC debug command per non-empty Voxy geometry bucket. This is still a debug command layout, not the formal renderer.")
                .define("mdicCommandBucketAware", true);
        MDIC_COMMAND_INCLUDE_TRANSLUCENT = builder
                .comment("Includes bucket 0 translucent records in bucket-aware MDIC debug commands. Disabled by default because translucent sorting is not implemented in this debug path.")
                .define("mdicCommandIncludeTranslucent", false);
        MDIC_COMMAND_INCLUDE_DOUBLE_SIDED = builder
                .comment("Includes bucket 1 double-sided records in bucket-aware MDIC debug commands.")
                .define("mdicCommandIncludeDoubleSided", true);
        MDIC_COMMAND_INCLUDE_DIRECTIONAL = builder
                .comment("Includes directional buckets 2..7 in bucket-aware MDIC debug commands.")
                .define("mdicCommandIncludeDirectional", true);
        MDIC_COMMAND_DIRECTIONAL_FACE_MASK = builder
                .comment("Filters directional MDIC bucket debug commands by camera position relative to the uploaded section AABB. This is conservative and still not occlusion culling.")
                .define("mdicCommandDirectionalFaceMask", true);
        MDIC_COMMAND_DIRECTIONAL_FACE_MASK_FALLBACK_ALL_WHEN_INSIDE = builder
                .comment("Keeps all directional buckets when the camera is inside a section AABB, avoiding missing faces in the debug renderer.")
        ...
        MDIC_COMMAND_SELECTION_MODE = builder
                .comment("MDIC debug command planning selection mode: AUTO, FRUSTUM_RADIUS, RADIUS, NEAREST_CAMERA, or FIRST_N.")
                .define("mdicCommandSelectionMode", "AUTO");
        MDIC_COMMAND_RENDER_DISTANCE_CHUNKS = builder
                .comment("Chunk radius used by MDIC debug command planning when RADIUS or FRUSTUM_RADIUS selection is active.")
                .defineInRange("mdicCommandRenderDistanceChunks", 16, 1, 128);
        MDIC_COMMAND_MAX_PLAN_CANDIDATES = builder
                .comment("Maximum uploaded section metadata entries inspected by one visibility-aware MDIC command planning pass.")
                .defineInRange("mdicCommandMaxPlanCandidates", 512, 1, 4096);
        MDIC_COMMAND_USE_FRUSTUM = builder
                .comment("Attempts to use a cached reliable frustum for MDIC debug command planning. Falls back when unavailable; no mixins or renderer integrations are added by this flag.")
        ...
        MDIC_COMMAND_MAX_COMMANDS = builder
                .comment("Maximum bucket-aware MDIC debug commands generated by one plan command.")
        ...
        ENABLE_MDIC_DEBUG_DRAW = builder
                .comment("G6.1 hardened minimal MDIC command-buffer debug draw. This reads the Forge MDIC skeleton command buffer and upload-only GL geometry heap only; it is not the real MDIC renderer.")
                .define("enableMdicDebugDraw", false);
        MDIC_DEBUG_DRAW_MAX_COMMANDS = builder
                .comment("Maximum MDIC skeleton commands drawn by the G6.x MDIC debug renderer.")
                .defineInRange("mdicDebugDrawMaxCommands", 256, 1, 512);
        ...
        MDIC_DEBUG_DRAW_MAX_DRAW_COUNT = builder
                .comment("Maximum draw count uploaded to the CPU-written G6.7 MDIC debug indirect count buffer.")
                .defineInRange("mdicDebugDrawMaxDrawCount", 256, 1, 512);
        ...
        MDIC_DEBUG_DRAW_IGNORE_DEPTH = builder
                .comment("Draws G6.1 MDIC debug quads through terrain to make the debug draw easier to see.")
                .define("mdicDebugDrawIgnoreDepth", true);
        MDIC_DEBUG_DRAW_DOUBLE_SIDED = builder
                .comment("Disables culling for the G6.1 hardened MDIC debug renderer while winding remains debug-only.")
                .define("mdicDebugDrawDoubleSided", true);
        MDIC_DEBUG_DRAW_FRAME_BUDGET_MS = builder
                .comment("Frame budget used by the G6.1 hardened MDIC debug draw timing status.")
                .defineInRange("mdicDebugDrawFrameBudgetMs", 4.0D, 0.1D, 100.0D);
        MDIC_DEBUG_DRAW_DEBUG_LOG = builder
                .comment("Logs G6.1 hardened minimal MDIC debug draw summaries.")
