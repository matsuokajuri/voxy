...
import me.cortex.voxy.client.core.vulkan.VulkanAsyncGeometryUpdates;
...
public class AsyncNodeManager {
    ...
    private final VulkanAsyncGeometryUpdates vulkanGeometryUpdates;
    ...
    public AsyncNodeManager(int maxNodeCount, IGeometryData geometryData, RenderGenerationService renderService) {
        ...
        this.vulkanGeometryUpdates = new VulkanAsyncGeometryUpdates();
    ...
    private void run() {
        ...
        do {
            ...
            long ptr = job.address;
            int zeroCount = 0;
            for (int i = 0; i < VulkanNodeCleaner.OUTPUT_COUNT; i++) {
                long pos = ((long) MemoryUtil.memGetInt(ptr)) << 32; ptr += 4;
                pos |= Integer.toUnsignedLong(MemoryUtil.memGetInt(ptr)); ptr += 4;
    ...
    public void tick(VoxyVulkanBuffer nodeBuffer, VulkanNodeCleaner cleaner) {//TODO: dont pass nodeBuffer here??, do something else thats better
        ...
        {//Update basic geometry data
            ...
            if (!upload.dataUploadPoints.isEmpty()) {
                ...
                this.vulkanGeometryUpdates.copyGeometry(
                        store, upload.scratchHeaderBuffer, copies, upload.scratchDataBuffer, scratchSize,
                        upload.maxElementAccess
                );
        ...
        if (!results.scatterWriteLocationMap.isEmpty()) {//Scatter write
            ...
            this.vulkanGeometryUpdates.scatter(
                    nodeBuffer, (BasicSectionGeometryData) this.geometryData,
                    results.scatterWriteBuffer, streamSize, count
            );
        }
    ...
    public synchronized void start() {
        if (this.stopped) throw new IllegalStateException("Cannot start a stopped AsyncNodeManager");
        this.thread.start();
    }
    ...
    public synchronized void stop() {
        ...
        this.vulkanGeometryUpdates.close();
