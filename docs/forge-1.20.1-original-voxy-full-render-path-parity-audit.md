...
| Shader semantics | adapter/subset shader is not full original terrain shader contract | Partially fixed in Roman VII/VIII: `ForgeOriginalVoxyRenderPipeline` now provides the original `AbstractRenderPipeline`-shaped shader hook owner, and `ForgeOriginalVoxyMdicSectionRenderer` builds original `quads3.vert` / `quads.frag` terrain and translucent programs through the original TAA hook, shader patch hook, directional face tint injection, and patched-or-normal fallback shape. The original `NormalRenderPipeline` target owner shape is now also present: opaque colour texture, SSAO/translucent colour texture, framebuffer attachment handoff, SSAO compute owner, original `SSAO.AUTO` capability selection, `postOpaquePreTranslucent(...)`, and final `transformBlitDepth(...)` handoff. Remaining gap: environmental fog uniforms and Oculus/Iris shaderpack patch semantics are not yet ported, so renderer readiness remains false. |
...
| `AbstractRenderPipeline` -> terrain shader owner | original `MDICSectionRenderer` receives a render pipeline, asks it for TAA and shader patches, compiles patched-or-normal opaque and translucent terrain programs, and then relies on the pipeline to bind opaque/translucent draw targets | Partially fixed in Roman VII: `ForgeOriginalVoxyRenderPipeline` owns the original-shaped shader hook boundary and reuses `ForgeOriginalVoxyPipelineDepthStage` for depth/stencil setup. `ForgeOriginalVoxyNormalPipelineTargets` now mirrors the original `NormalRenderPipeline` colour target ownership enough for opaque and translucent target binding: the Voxy depth/stencil framebuffer receives the opaque colour texture, and the separate SSAO framebuffer receives the shared depth attachment plus SSAO colour texture. `ForgeOriginalVoxySSAO` mirrors original SSAO shader compilation, sampler setup, AUTO capability selection, matrix uniforms, image/texture bindings, and dispatch. The final blit shader source owner is present with the original `EMIT_COLOUR` define, and `ForgeOriginalVoxyRenderPipeline.finish(...)` now mirrors `AbstractRenderPipeline.transformBlitDepth(...)`. Remaining gap: environmental fog and Oculus shaderpack patch integration are not yet ported because the Forge Embeddium hook does not currently expose original `Viewport.fogParameters`, and the Oculus pipeline-data bridge equivalent to `IrisVoxyRenderPipelineData` has not been ported. |
...
validate HiZ-backed HierarchicalOcclusionTraverser runtime request batches
 ...
 -> port original render-pipeline environmental fog and Oculus shaderpack patch semantics
