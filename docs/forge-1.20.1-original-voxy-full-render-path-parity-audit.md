...
| `MDICViewport` -> `prep.comp` / cull raster / `cmdgen.comp` | original `MDICSectionRenderer.buildDrawCalls(...)` uploads the MDIC scene uniform, runs `prep.comp`, rasterizes section AABBs into the viewport visibility buffer with color/depth writes disabled, then dispatches production `cmdgen.comp` indirectly from `drawCountCallBuffer` | Fixed for the active Roman V owner route at the opaque/cutout command-generation level: `ForgeOriginalVoxyMdicCommandGenerator` owns original-shaped uniform, distance-count, shared-index, prep, cull-raster, and cmdgen resources. It binds real `ForgeOriginalVoxyMdicViewport` buffers and real `ForgeOriginalVoxyBasicSectionGeometryData` metadata/geometry buffers, not K-era synthetic validators or debug command buffers. Runtime readback after the depth/stencil parity fix verifies a non-empty render-list (`renderListSectionCount=146`), production `cmdgen.comp` output (`opaqueDrawCount=545`), original draw-count layout, cull indirect command layout, 20-byte `DrawCommand`, position scratch output, and barrier path. It deliberately does not call `MDICSectionRenderer`, does not call `VoxyRenderSystem`, and does not submit terrain draw commands. Remaining adapter: the cull raster pass runs inside the Forge render hook with state restoration because the original `AbstractRenderPipeline` / `VoxyRenderSystem` owner is not yet ported. Remaining unported original `buildDrawCalls(...)` tail: translucent prefix sum and `buildtranslucents.comp` output. |
...
validate HiZ-backed HierarchicalOcclusionTraverser runtime request batches
 ...
 -> runtime-audit production cmdgen.comp output
 -> port MDICSectionRenderer draw setup and original terrain shader bindings
